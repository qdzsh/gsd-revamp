...
test("Feature 11: theme switching JS is not included", () => {
  const html = generateHtmlReport(mockData(), mockOpts());
  assert.ok(!html.includes(["theme", "toggle"].join("-")), "should not contain theme toggle class");
  assert.ok(!html.includes(["gsd", "theme"].join("-")), "should not reference localStorage key for theme");
  assert.ok(!html.includes(["light", "theme"].join("-")), "should not reference light theme class");
});
...
test("Edge: runtime light theme CSS variables are not defined", () => {
  const html = generateHtmlReport(mockData(), mockOpts());
  assert.ok(!html.includes(`.${["light", "theme"].join("-")}{`), "should not include light theme CSS rule");
});
