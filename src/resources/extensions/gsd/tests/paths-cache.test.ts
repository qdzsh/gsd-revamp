...
describe('gsdRootCache key normalization', () => {
  ...
  beforeEach(() => {
    projectDir = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-cache-norm-')));
    mkdirSync(join(projectDir, '.gsd'), { recursive: true });

    fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-cache-home-')));
...
describe('clearPathCache() does NOT invalidate gsdRootCache (process-lifetime semantics)', () => {
  ...
  beforeEach(() => {
    projectDir = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-cache-clear-')));
    mkdirSync(join(projectDir, '.gsd'), { recursive: true });

    fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-cache-home2-')));
