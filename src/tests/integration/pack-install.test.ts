...
test("tarball installs and gsd binary resolves", async (t) => {
  ...
  const installedLoader = join(sandbox.installPrefix, "node_modules", "gsd-revamp", "dist", "loader.js");
...
test("gsd exits early with a clear message when synced resources are newer than the binary", async (t) => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ gsdVersion: "999.0.0", packageName: "gsd-revamp" }),
  );
  ...
  assert.match(result.stderr, /npm install -g gsd-revamp@latest|gsd upgrade/, "prints upgrade guidance");
