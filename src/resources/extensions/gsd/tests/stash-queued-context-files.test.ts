...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-ctx-stash-test-")));
  run("git init", dir);
...
function createTempRepoWithSymlinkedGsd(): { repo: string; stateDir: string } {
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-symlink-stash-test-")));
  const stateDir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-symlink-state-")));
  run("git init", repo);
...
test("#2505: git stash --include-untracked sweeps queued CONTEXT files (demonstrates the bug)", () => {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-stash-bug-demo-")));
  try {
    run("git init", dir);
    run("git config user.email test@test.com", dir);
    run("git config user.name Test", dir);
...
test("#4573: gitignored .gsd symlink does not break pre-merge stash", () => {
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-4573-ignored-symlink-")));
  const stateDir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-4573-state-")));
  try {
    run("git init", repo);
    run("git config user.email test@test.com", repo);
    run("git config user.name Test", repo);
