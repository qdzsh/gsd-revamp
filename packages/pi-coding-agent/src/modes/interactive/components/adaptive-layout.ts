...
import { truncateToWidth, type Component } from "@gsd/pi-tui";
...
export class AdaptiveLayoutComponent implements Component {
	...
	private frame(lines: string[], width: number, title: string, rightTitle: string, accent: ThemeColor): string[] {
		...
		return [
			rightAlign(theme.fg("surfaceTitle", title), theme.fg(accent, rightTitle), outerWidth),
			...lines.map((line) => truncateToWidth(line, outerWidth, "")),
		];
	}
