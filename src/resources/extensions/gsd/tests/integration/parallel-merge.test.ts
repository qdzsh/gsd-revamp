...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "parallel-merge-test-")));
...
test("determineMergeOrder — finds milestones completed in canonical DB even when worker state is 'error' (#2812)", () => {
  const base = realpathSync.native(mkdtempSync(join(tmpdir(), "merge-db-bug-")));
  try {
...
test("determineMergeOrder — combines stopped workers and DB-complete milestones without duplicates", () => {
  const base = realpathSync.native(mkdtempSync(join(tmpdir(), "merge-dedup-")));
  try {
