// Project/App: GSD-2
// File Purpose: Tests for the GSD health widget state rendering.
...
test("buildHealthLines: none state shows single onboarding line pointing at /gsd", (t) => {
  ...
  assert.equal(lines[0], "GSD  No project loaded — run /gsd to start");
  // Should not show active-project chrome when there's no project.
  assert.ok(!/Spent|Last commit/.test(lines[0]!), "no active-project chrome");
...
test("buildHealthLines: active state with ledger-driven spend shows spent summary", (t) => {
  ...
  assert.equal(lines[0], "Spent: $0.42");
});

test("buildHealthLines: active state with budget ceiling still shows only spend", (t) => {
  const lines = buildHealthLines(activeData({ budgetSpent: 2.5, budgetCeiling: 10 }));
  assert.equal(lines.length, 1);
  assert.equal(lines[0], "Spent: $2.50");
});

test("buildHealthLines: active state omits issue and environment summaries", (t) => {
  const lines = buildHealthLines(activeData({
    budgetSpent: 61.86,
    providerIssue: "✗ OpenAI key missing",
    environmentErrorCount: 1,
    environmentWarningCount: 2,
  }));
  assert.equal(lines.length, 1);
  assert.equal(lines[0], "Spent: $61.86");
  assert.doesNotMatch(lines[0]!, /warning|Env:|OpenAI|System OK|issue|⚠|✗/);
});

test("buildHealthLines: warning status is not rendered in active compact line", (t) => {
  const lines = buildHealthLines(activeData({ environmentWarningCount: 1 }));
  assert.equal(lines.length, 1);
  assert.equal(lines[0], "Spent: $0.00");
});
...
test("buildHealthLines: shows last commit with relative time and message", (t) => {
  ...
  assert.match(lines[0]!, /Spent: \$0\.00 │ Last commit: 5m ago/);
  assert.match(lines[0]!, /feat\(widget\): add health display/);
});

test("buildHealthLines: cost uses comma grouping and exactly 2 decimals", (t) => {
  const lines = buildHealthLines(activeData({ budgetSpent: 99345.12 }));
  assert.equal(lines[0], "Spent: $99,345.12");
});

test("buildHealthLines: does not truncate long commit messages without terminal width", (t) => {
  ...
  assert.equal(lines.length, 1);
  assert.ok(lines[0]!.includes(longMsg), "full message should be present when width is unknown");
  assert.ok(!lines[0]!.includes("…"), "widthless rendering should not add ellipsis");
});

test("buildHealthLines: truncates by terminal width when width is provided", (t) => {
  const epoch = Math.floor(Date.now() / 1000) - 60;
  const longMsg = "a".repeat(200);
  const width = 80;
  const lines = buildHealthLines(activeData({
  ...
  assert.ok(lines[0]!.endsWith("…"), "terminal-width truncation should add ellipsis");
  assert.ok(!lines[0]!.includes(longMsg), "full message should not appear when terminal width is insufficient");
});
