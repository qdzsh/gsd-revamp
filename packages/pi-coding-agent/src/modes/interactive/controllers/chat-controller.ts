...
let streamingRenderTimer: ReturnType<typeof setTimeout> | undefined;
let lastStreamingRenderAt = 0;

function requestStreamingRender(host: InteractiveModeStateHost): void {
	const now = Date.now();
	const elapsed = now - lastStreamingRenderAt;

	if (elapsed >= STREAMING_RENDER_MIN_INTERVAL_MS) {
		lastStreamingRenderAt = now;
		host.ui.requestRender();
		return;
	}

	if (streamingRenderTimer !== undefined) return;
	streamingRenderTimer = setTimeout(() => {
		streamingRenderTimer = undefined;
		lastStreamingRenderAt = Date.now();
		host.ui.requestRender();
	}, STREAMING_RENDER_MIN_INTERVAL_MS - elapsed);
}

function cancelPendingStreamingRender(): void {
	if (streamingRenderTimer === undefined) return;
...
function replaceCompactToolRowsWithPhaseSummary(
	...
	const flushRollupRun = () => {
		...
		const components = rollupRun.flatMap((item) => item.components);
		const summary = new ToolPhaseSummaryComponent(phases, components);
	...
	for (const seg of renderedSegments) {
		...
		if (seg.kind === "tool" && phase) {
			rollupRun.push({ seg, phases: [phase], components: [seg.component] });
			continue;
		}
		if (seg.kind === "tool-summary") {
			rollupRun.push({
				seg,
				phases: seg.component.getPhases(),
				components: seg.component.getRolledUpComponents(),
			});
			continue;
		}
