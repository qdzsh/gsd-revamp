...
1. **Search closed issues** for similar keywords:
   \`\`\`
   gh issue list --repo qdzsh/gsd-revamp --state closed --search "<keywords from root cause>" --limit 20
   \`\`\`

2. **Search open PRs** that might contain the fix:
   \`\`\`
   gh pr list --repo qdzsh/gsd-revamp --state open --search "<keywords>" --limit 10
   \`\`\`

3. **Search merged PRs** that may have already fixed this:
   \`\`\`
   gh pr list --repo qdzsh/gsd-revamp --state merged --search "<keywords>" --limit 10
   \`\`\`
