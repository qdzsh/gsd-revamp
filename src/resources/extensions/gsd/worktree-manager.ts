...
export function listWorktrees(basePath: string): WorktreeInfo[] {
  ...
  for (const entry of entries) {
    ...
    const normalizedBranchWorktreeName = branchWorktreeName && process.platform === "win32"
      ? branchWorktreeName.toLowerCase()
      : branchWorktreeName;
    const matchesBranchLeaf = normalizedBranchWorktreeName
      ? normalizedEntryVariants.some(entryVariant => entryVariant.split("/").pop() === normalizedBranchWorktreeName)
      : false;
