/**
 * Shared utilities for Google Generative AI providers (Gemini and Vertex AI).
 */
...
/**
 * Thinking levels accepted by Google's generation config (mirrors Google's
 * `ThinkingLevel` enum values). Shared across the Gemini and Vertex providers.
 */
...
export function convertMessages<T extends GoogleApiType>(model: Model<T>, context: Context): Content[] {
	...
	for (const msg of transformedMessages) {
		if (msg.role === "user") {
			...
			// Google APIs require all function responses to be in a single user turn.
 ...
 *
 * This is needed when proxying Claude models through Google APIs,
 * since they use a restricted subset of JSON Schema.
 */
 ...
 * field instead (OpenAPI 3.03 Schema). This is needed for Google APIs serving Claude
