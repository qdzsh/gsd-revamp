...
describe("project-relocation-recovery (#2750)", () => {
  ...
  before(() => {
    ...
    stateDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-state-")));
  ...
  test("repoIdentity is stable across moves for repos with a remote URL", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-a-")));
  ...
  test("ensureGsdSymlink reuses the same external dir after repo move (remote repo)", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-reuse-a-")));
  ...
  test("repo-meta.json gitRoot is updated after move (remote repo)", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-meta-a-")));
  ...
  test("ensureGsdSymlink writes a .gsd-id marker in the project root", () => {
    const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-marker-")));
    initRepo(repo);
  ...
  test("local-only repo recovers state via .gsd-id marker after move", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-local-a-")));
    initRepo(repoA);
  ...
  test("identity remains different for repos with different remotes", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-diff-a-")));
    initRepo(repoA, "https://github.com/example/repo-alpha.git");

    const repoB = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-diff-b-")));
  ...
  test("no orphaned state dir created when remote repo is moved", () => {
    const repoA = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-reloc-orphan-a-")));
