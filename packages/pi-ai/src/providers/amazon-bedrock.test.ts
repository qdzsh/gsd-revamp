...
describe("supportsAdaptiveThinking — regression guard #4392 / #4352: all supported models", () => {
    ...
    it("returns true for opus-4-8 (hyphen, Bedrock ARN style)", () => {
        assert.ok(
            supportsAdaptiveThinking("anthropic.claude-opus-4-8-20250514-v1:0"),
            "opus-4-8 should support adaptive thinking",
        );
    });

    it("returns true for opus-4-8 (dot separator)", () => {
        assert.ok(
            supportsAdaptiveThinking("anthropic.claude-opus-4.8-20250514-v1:0"),
            "opus-4.8 (dot) should support adaptive thinking",
        );
    });
...
describe("buildAdditionalModelRequestFields — Bug #4392: opus-4-7 must use adaptive thinking", () => {
    ...
    it("opus-4-8 Bedrock ARN → thinking.type === 'adaptive'", () => {
        const model = makeModel("anthropic.claude-opus-4-8-20250514-v1:0");
        const fields = buildAdditionalModelRequestFields(model, options);
        assert.equal(
            fields?.thinking?.type,
            "adaptive",
            "opus-4-8 should produce thinking.type='adaptive', not budget_tokens",
        );
    });
...
describe("mapThinkingLevelToEffort — regression guard #4392: opus-4-7 xhigh returns 'xhigh'", () => {
    ...
    it("maps xhigh → 'xhigh' for opus-4-8 (native xhigh support)", () => {
        const result = mapThinkingLevelToEffort("xhigh", "anthropic.claude-opus-4-8-20250514-v1:0");
        assert.equal(
            result,
            "xhigh",
            "opus-4-8 supports native xhigh effort — must not be clamped to 'max'",
        );
    });
