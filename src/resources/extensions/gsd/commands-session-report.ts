...
function formatSessionReport(units: UnitMetrics[]): string {
  ...
  lines.push(`Units:       ${String(units.length)}`);
  lines.push(`Cost:        ${formatCost(totals.cost)}`);
  lines.push(`Tokens:      ${formatTokenCount(totals.tokens.input)} in / ${formatTokenCount(totals.tokens.output)} out`);
  ...
  if (units.length > 0) {
    ...
    for (const unit of units) {
      ...
      lines.push(`  ${status} ${unit.id ?? "unknown"}`);
    }
  }
  ...
  if (byModel.length > 0) {
    ...
    for (const m of byModel) {
      lines.push(`  ${m.model}: ${m.units} units (${formatCost(m.cost)})`);
    }
  }
