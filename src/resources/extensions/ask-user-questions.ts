 ...
 * or multi-select (allowMultiple: true). A free-form "Type something." option
...
const QuestionSchema = Type.Object({
	...
	options: Type.Array(OptionSchema, {
		description:
			'Provide 2-3 mutually exclusive choices for single-select, or any number for multi-select. Put the recommended option first and suffix its label with "(Recommended)". Each option can include an optional "preview" field with markdown content shown in a side panel. Do not include an "Other" option for single-select; the client adds a free-form "Type something." option automatically.',
	}),
	allowMultiple: Type.Optional(
		Type.Boolean({
			description:
				"If true, the user can select multiple options using SPACE to toggle and ENTER to confirm. No 'Type something.' option is added. Default: false.",
		}),
	),
});
...
export default function AskUserQuestions(pi: ExtensionAPI) {
	pi.registerTool({
		...
		promptGuidelines: [
			...
			"For single-select: each question must have 2-3 options. Put the recommended option first with '(Recommended)' suffix. Do not include an 'Other' or 'Type something.' option - the client adds one automatically.",
			"For multi-select: set allowMultiple: true. The user can pick any number of options. No 'Type something.' option is added.",
		...
		async execute(_toolCallId, params, signal, _onUpdate, ctx) {
			...
			if (!result) {
				...
				for (const q of params.questions) {
					...
					if (!q.allowMultiple) {
						options.push(TYPE_SOMETHING_LABEL);
					}
					...
					// When the user picks "Type something." on a single-select
					...
					if (!q.allowMultiple && selectedStr === TYPE_SOMETHING_LABEL) {
		...
		renderCall(args, theme) {
			...
			for (const q of qs) {
				...
				const optLabels = multiSel
					...
					: [...(q.options || []).map((o: QuestionOption) => o.label), TYPE_SOMETHING_LABEL];
