...
# File Purpose: Define runtime container images.

# ──────────────────────────────────────────────
# Runtime
# Image: ghcr.io/qdzsh/gsd-revamp
...
RUN npm install -g gsd-revamp@${GSD_VERSION}
...
# ──────────────────────────────────────────────
# Runtime (local build)
# Image: ghcr.io/qdzsh/gsd-revamp:local
...
#                --build-arg TARBALL=gsd-revamp-<version>.tgz -t gsd-revamp:local .
...
ARG TARBALL
COPY ${TARBALL} /tmp/gsd-revamp.tgz
...
RUN npm install -g --ignore-scripts /tmp/gsd-revamp.tgz \
    && rm /tmp/gsd-revamp.tgz \
    && echo "--- /usr/local/bin ---" \
    && ls -la /usr/local/bin | grep -i gsd || echo "(no gsd entries in /usr/local/bin)" \
    && echo "--- /usr/local/lib/node_modules/gsd-revamp ---" \
    && ls -la /usr/local/lib/node_modules/gsd-revamp 2>/dev/null | head -10 \
    && test -f /usr/local/lib/node_modules/gsd-revamp/dist/loader.js \
    && node /usr/local/lib/node_modules/gsd-revamp/dist/loader.js --version
...
ENTRYPOINT ["node", "/usr/local/lib/node_modules/gsd-revamp/dist/loader.js"]
CMD ["--help"]
