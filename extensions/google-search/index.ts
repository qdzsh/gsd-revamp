...
export default function (pi: ExtensionAPI) {
			...
			if (!process.env.GEMINI_API_KEY) {
				return {
					content: [
						{
							type: "text",
							text: "Error: No authentication found for Google Search. Please set GEMINI_API_KEY.\n\nExample: export GEMINI_API_KEY=your_key",
						},
					],
					isError: true,
					details: {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		ctx.ui.notify(
			"Google Search: No authentication set. Set GEMINI_API_KEY to use google_search.",
			"warning",
		);
	});
}
