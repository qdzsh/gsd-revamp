...
export function hasProjectFileInAncestor(
  ...
  const pathExists = (path: string): boolean => existsFn(path) || existsFn(path.replaceAll("\\", "/"));
  let checkDir = dirname(startDir);
  const { root } = parsePath(checkDir);
  while (checkDir !== root) {
    if (PROJECT_FILES.some((f) => pathExists(join(checkDir, f)))) {
      return true;
    }
    ...
    if (pathExists(join(checkDir, ".git"))) return false;
...
export function hasGsdBootstrapArtifacts(
  ...
  return (
    pathExists(gsdPath) &&
    (pathExists(join(gsdPath, "PREFERENCES.md")) ||
      pathExists(join(gsdPath, "preferences.md")) ||
      pathExists(join(gsdPath, "milestones")))
  );
}
