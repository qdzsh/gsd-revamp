...
function sendPrompt(description: string, result: RoundResult, pi: ExtensionAPI): void {
	...
	if (uiSelected.includes("tool") || result.answers["purpose"]) {
		docHints.push("- `docs/extension-sdk/building-extensions.md#tool-rendering-optional` — renderCall / renderResult");
	}

	if (eventsSelected && !eventsSelected.includes("standalone")) {
		docHints.push("- `docs/extension-sdk/building-extensions.md#events-lifecycle-hooks` — event hooks reference");
	}

	if (eventsSelected.includes("context / prompt")) {
		docHints.push("- `docs/extension-sdk/building-extensions.md#system-prompt-modification` — system prompt hooks");
	}
