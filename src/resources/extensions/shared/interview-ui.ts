 ...
 * - Optional notes field (n to open)
 ...
 *   n            open notes field
 *   Tab          switch questions when there are multiple questions
...
const TYPE_SOMETHING_DESCRIPTION = "Type your own answer.";
...
export async function showInterviewRound(
	...
	return ctx.ui.custom<RoundResult>((tui: TUI, theme: Theme, _kb, done) => {
		...
		function totalOpts(qIdx: number): number {
			return questions[qIdx].options.length + 2;
		}

		function typeOrDoneIdx(qIdx: number): number {
			return questions[qIdx].options.length;
		}

		function chatIdx(qIdx: number): number {
		...
		function buildResult(): RoundResult {
			...
			for (let i = 0; i < questions.length; i++) {
				...
				if (isMultiSelect(i)) {
					...
					if (effectiveCommittedIndex === chatIdx(i)) continue;
					let selected = TYPE_SOMETHING_LABEL;
					if (effectiveCommittedIndex !== null) {
						const idx = effectiveCommittedIndex;
						if (idx < q.options.length) selected = q.options[idx].label;
						else if (idx === typeOrDoneIdx(i)) selected = TYPE_SOMETHING_LABEL;
					}
		...
		function goNextOrSubmit() {
			if (states[currentIdx].cursorIndex === chatIdx(currentIdx)) {
			...
			// Auto-open the notes field when "Type something." is selected
			...
			if (!isMultiSelect(currentIdx) && states[currentIdx].cursorIndex === typeOrDoneIdx(currentIdx) && !states[currentIdx].notes && !states[currentIdx].notesVisible) {
		...
		function handleInput(data: string) {
			...
			if (showingReview) {
				...
				if (isMultiQuestion && matchesKey(data, Key.tab)) {
					showingReview = false;
					switchQuestion(0);
					return;
				}
			...
			if (focusNotes) {
				...
				if (matchesKey(data, Key.enter)) {
					saveEditorToState();
					focusNotes = false;
					if (!multiSel && st.committedIndex === null) st.committedIndex = typeOrDoneIdx(currentIdx);
					goNextOrSubmit();
					return;
				}
			...
			if (multiSel) {
				const doneI = typeOrDoneIdx(currentIdx);
				const chatI = chatIdx(currentIdx);
				if (matchesKey(data, Key.space)) {
					...
					if (st.cursorIndex === chatI) finish({ endInterview: false, answers: {} });
					return;
				}
				if (matchesKey(data, Key.enter)) { goNextOrSubmit(); return; }
				if (data.toLowerCase() === "n") { st.notesVisible = true; focusNotes = true; loadStateToEditor(); refresh(); return; }
			} else {
				...
				if (!isMultiQuestion && matchesKey(data, Key.tab)) { st.notesVisible = true; focusNotes = true; loadStateToEditor(); refresh(); return; }
				if (data.toLowerCase() === "n") { st.notesVisible = true; focusNotes = true; loadStateToEditor(); refresh(); return; }
				if (matchesKey(data, Key.enter)) { goNextOrSubmit(); return; }
			}
		}
		...
		function renderReviewScreen(width: number): string[] {
			...
			if (isMultiQuestion) {
				...
				push(ui.questionTabs(questions.map(q => q.header), questions.length, answeredSet, { includeSubmit: true }));
				push(ui.blank());
			}
			push(ui.header(`  ${opts.reviewHeadline ?? "Review your answers"}`), ui.blank());
			...
			for (let i = 0; i < questions.length; i++) {
				...
				if (isMultiSelect(i)) {
					...
					const effectiveCommittedIndex = st.committedIndex ?? st.cursorIndex;
					let label = TYPE_SOMETHING_LABEL;
			...
			push(
				...
				ui.hints([
					"Enter to submit",
					isMultiQuestion ? "Tab to switch questions" : "← to go back and edit",
					`Esc to ${opts.exitLabel ?? "cancel"}`,
				]),
				ui.bar(),
			);
		...
		function renderOptionsColumn(optWidth: number): string[] {
			...
			const ndIdx = typeOrDoneIdx(currentIdx);
			...
			if (multiSel) {
				...
				if (ndCursor && !focusNotes) {
					push(ui.actionSelected(ndIdx + 1, TYPE_SOMETHING_LABEL, TYPE_SOMETHING_DESCRIPTION));
				} else {
					push(ui.actionUnselected(ndIdx + 1, TYPE_SOMETHING_LABEL, TYPE_SOMETHING_DESCRIPTION));
				}
			}

			const chatI = chatIdx(currentIdx);
			const chatCursor = chatI === st.cursorIndex;
			push(ui.blank());
			if (chatCursor && !focusNotes) push(ui.actionSelected(chatI + 1, CHAT_LABEL));
			else push(ui.actionUnselected(chatI + 1, CHAT_LABEL));
		...
		function render(width: number): string[] {
			...
			if (useSideBySide) {
				...
				if (isMultiQuestion) {
					...
					const answeredSet = new Set(questions.map((_, i) => i).filter(i => isQuestionAnswered(i)));
					push(ui.questionTabs(questions.map(q => q.header), currentIdx, answeredSet, { includeSubmit: true }));
					push(ui.blank());
					const progressParts = [
						opts.progress,
				...
				if (focusNotes) {
					...
					hints.push("n to add notes");
					if (isMultiQuestion) hints.push("Tab to switch questions");
				} else {
					hints.push("Enter to select");
