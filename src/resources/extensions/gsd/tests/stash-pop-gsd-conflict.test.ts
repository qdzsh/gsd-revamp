...
test.before(() => {
  ...
  fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-fake-home-")));
...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-stashpop-test-")));
  run("git init", dir);
...
test("#2766: stash pop conflict on non-.gsd files preserves local work for manual resolution", () => {
  const repo = createTempRepo();
  try {
    ...
    let err: unknown;
    try {
      mergeMilestoneToMain(repo, "M301", roadmap);
    } catch (caught) {
      err = caught;
    }
    assert.ok(err instanceof Error, "non-.gsd stash pop conflicts fail closed for manual resolution");
    assert.match(err.message, /README\.md/);
    assert.equal((err as { code?: string }).code, "GSD_MERGE_CONFLICT");

    // Non-.gsd conflicts must keep the user's local work available for manual resolution.
    const stashList = run("git stash list", repo);
    const readme = readFileSync(join(repo, "README.md"), "utf-8");
    assert.ok(
      stashList.length > 0 || readme.includes("locally modified"),
      "local work is preserved either in the stash or restored into the working tree",
    );
  } finally {
    cleanupTempRepo(repo);
  }
