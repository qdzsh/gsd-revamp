...
test("pnpm layout: merged node_modules contains entries from both hoisted and internal", (t) => {
  ...
  mkdirSync(join(internal, "@gsd-revamp", "core"), { recursive: true });
  ...
  assert.ok(existsSync(join(agentNodeModules, "@gsd-revamp")), "@gsd-revamp should resolve");
...
test("hasMissingWorkspaceScopes detects pnpm layout", (t) => {
  ...
  // pnpm-style: @gsd-revamp only in internal
  mkdirSync(join(internal, "@gsd-revamp"), { recursive: true });
  assert.equal(
    hasMissingWorkspaceScopes(hoisted, internal),
    true,
    "pnpm-style: @gsd-revamp missing from hoisted should be detected",
  );
