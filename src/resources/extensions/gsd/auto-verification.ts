  ...
  try {
    ...
    if (result.passed) {
      ...
      if (unitCostUsd >= perUnitCapUsd) {
        ...
        await pauseAuto(ctx, pi, {
          message: `Verification failed and unit ${s.currentUnit.id} hit per-unit cap ${formatCost(perUnitCapUsd)}.`,
          category: "unknown",
        });
        return "pause";
      }
      if (getUnitCostSpikeAction(unitCostUsd, rollingAvgUsd, 3.0) === "pause") {
        ctx.ui.notify(
          `Unit ${s.currentUnit.id} cost spike detected (${formatCost(unitCostUsd)} vs avg ${formatCost(rollingAvgUsd)}) during verification retry; keeping verification failure as the authoritative blocker.`,
          "warning",
        );
      }
      const nextAttempt = attempt + 1;
