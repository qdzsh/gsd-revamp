...
import { normalizeVerifyString, validateVerificationCommand } from "./verification-gate.js";
...
export function checkVerificationCommands(tasks: TaskRow[]): PreExecutionCheckJSON[] {
  ...
  for (const task of tasks) {
    const verify = normalizeVerifyString(task.verify).verify.trim();
    if (!verify) continue;
...
export function extractPackageReferences(description: string): string[] {
  ...
  // See: https://github.com/qdzsh/gsd-revamp/issues/4388
