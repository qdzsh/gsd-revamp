 ...
 * ask-user-questions.ts to ensure that selecting "Type something."
...
describe("ask-user-questions RPC fallback free-text", () => {
	...
	it("registers the canonical ask_user_questions tool only", () => {
		const tool = captureTool();
		assert.equal(tool.name, "ask_user_questions");
	});

	it("prompts for free-text input when user selects 'Type something.'", async () => {
		const tool = captureTool();
		...
		// The select should have been called with "Type something." appended
		assert.equal(selectCalls.length, 1);
		assert.ok(
			selectCalls[0].options.includes("Type something."),
			"select options should include 'Type something.'",
		);

		// A follow-up input() call should have been made to collect free text
		assert.equal(inputCalls.length, 1, "should call ctx.ui.input() for free-text after 'Type something.'");
	...
	it("handles cancelled free-text input gracefully", async () => {
		...
		// Result should still contain "Type something." without a note
		const text = result.content[0]?.text;
		assert.ok(text, "result should have text content");
		const parsed = JSON.parse(text);
		assert.deepStrictEqual(parsed.answers.q1.answers, ["Type something."]);
	});
});
