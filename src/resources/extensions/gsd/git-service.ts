...
export function createDraftPR(
  ...
  try {
    ...
    const env = opts?.env ?? GIT_NO_PROMPT_ENV;
    const command = resolveCommandFromPath("gh", env);
    const invocation = buildWindowsShimInvocation(command, args);
...
function buildWindowsShimInvocation(command: string, args: string[]): { file: string; args: string[] } {
