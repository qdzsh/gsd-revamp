...
import { formatCost, getLedger } from "./metrics.js";
  ...
  try {
    ...
    if (skipLifecycleCommit) {
      ...
      if (gitResult.status === "failed") {
        ...
        const isHuskyLint = /husky.*pre-commit script failed|pre-commit hook failed|lint-staged/i.test(fullError);
        const maxLintFixAttempts = 3;
        if (
          turnAction === "commit" &&
          isHuskyLint &&
          s.lintFixAttempts < maxLintFixAttempts &&
          unit.id
        ) {
          s.lintFixAttempts += 1;
          s.lastGitActionFailure = null;
          ...
          const fixPrompt = [
            "Lint errors blocked the previous commit.",
            "Read the errors below, fix every reported issue in the listed files, and stage the changes with `git add` so the next commit succeeds.",
            "If a rule is not auto-fixable, refactor the code so the rule passes. Do not disable lint rules. Do not bypass the hook.",
            "Do not commit yourself; GSD will re-attempt the commit automatically after you finish.",
            "",
            `Original unit: ${unit.type} ${unit.id} (attempt ${s.lintFixAttempts}/${maxLintFixAttempts})`,
            "",
            "Failure context (stderr, last 4000 chars):",
            "```",
            fullError.slice(-4000),
            "```",
          ].join("\n");
          ctx.ui.notify(
            `Lint blocked commit (attempt ${s.lintFixAttempts}/${maxLintFixAttempts}) — auto-remediating...`,
            "warning",
          );
          debugLog("postUnit", {
            phase: "commit-autofix-enqueue",
            attempt: s.lintFixAttempts,
            originalUnit: unit.id,
            errorPreview: fullError.slice(0, 200),
          });
          return enqueueSidecar(s, ctx, {
            kind: "quick-task",
            unitType: "quick-task",
            unitId: `${unit.id}/fix-lint-${s.lintFixAttempts}`,
            prompt: fixPrompt,
          }, {
            originalUnit: unit.id,
            attempt: s.lintFixAttempts,
            reason: "husky-pre-commit-lint",
          }, `Auto-fix lint (attempt ${s.lintFixAttempts}/${maxLintFixAttempts})`);
        }
        if (turnAction === "commit" && isHuskyLint && s.lintFixAttempts >= maxLintFixAttempts) {
          debugLog("postUnit", {
            phase: "commit-autofix-exhausted",
            attempts: s.lintFixAttempts,
            originalUnit: unit.id,
          });
        }
        s.lastGitActionFailure = fullError;
            ...
            if (sMid && sSid && sTid && isDbAvailable()) {
              ...
              if (taskRow?.status === "complete") {
                ...
                if (blockingMismatch) {
                  ...
                  const nextAttempt = (s.verificationRetryCount.get(retryKey) ?? 0) + 1;
                  const mismatchReasons = mismatches
                    .filter((mismatch) => mismatch.severity === "error")
                    .map((mismatch) => mismatch.reason);
                  const mismatchSummary = mismatchReasons.length > 0
                    ? mismatchReasons.slice(0, 5).map((reason) => `- ${reason}`).join("\n")
                    : "- Verification evidence mismatch detected.";

                  if (autoFixEnabled && nextAttempt <= maxRetries) {
                    let stateResetReady = true;
                    try {
                      updateTaskStatus(sMid, sSid, sTid, "pending");
                      await renderPlanCheckboxes(s.canonicalProjectRoot, sMid, sSid);
                    } catch (dbErr) {
                      stateResetReady = false;
                      logError("engine", `verification autorecover state-reset failed for ${sMid}/${sSid}/${sTid}: ${(dbErr as Error).message}. Run 'gsd recover' to reconcile.`);
                    }
                    try {
                      const tasksDir = resolveTasksDir(s.canonicalProjectRoot, sMid, sSid);
                      if (!tasksDir) {
                        stateResetReady = false;
                        logError("engine", `verification autorecover could not resolve tasks dir for ${sMid}/${sSid}/${sTid}`);
                      } else {
                        const summaryFile = join(tasksDir, buildTaskFileName(sTid, "SUMMARY"));
                        if (existsSync(summaryFile)) unlinkSync(summaryFile);
                      }
                    } catch (fsErr) {
                      stateResetReady = false;
                      logError("engine", `verification autorecover summary cleanup failed for ${sMid}/${sSid}/${sTid}: ${(fsErr as Error).message}`);
                    }
                    if (!stateResetReady) {
                    ...
                    s.pendingVerificationRetry = {
                      ...
                      attempt: nextAttempt,
                      failureContext: [
                        "Safety evidence cross-reference found a mismatch between recorded execution and claimed verification evidence.",
                        "Fix the underlying verification mismatch before completing the task again.",
                        "",
                        `Task: ${sMid}/${sSid}/${sTid}`,
                        "",
                        "Blocking mismatch details:",
                        mismatchSummary,
                      ].join("\n"),
                    };
                    try {
                      ...
                      debugLog("postUnit", { phase: "safety-evidence-xref-clear", error: String(clearErr) });
                    }
                    invalidateAllCaches();
                    debugLog("postUnit", {
                      phase: "safety-evidence-xref-retry",
                    ...
                    ctx.ui.notify(
                      `Safety: task ${sTid} claimed verification evidence that did not match recorded execution — retrying (${nextAttempt}/${maxRetries})`,
                      "warning",
                    );
                    return "retry";
                  }
      ...
      if (!triggerArtifactVerified && USER_DRIVEN_DEEP_UNITS.has(s.currentUnit.type) && isAwaitingUserInput(opts?.agentEndMessages)) {
        ...
        if (hasExpectedArtifact) {
          ...
          if (unitCostUsd >= perUnitCapUsd) {
            ...
            s.verificationRetryFailureHashes.delete(retryKey);
            ctx.ui.notify(
              `Unit ${s.currentUnit.id} hit per-unit cap ${formatCost(perUnitCapUsd)} — pausing auto-mode.`,
              "error",
            );
            await pauseAuto(ctx, pi);
            return "dispatched";
          ...
          if (getUnitCostSpikeAction(unitCostUsd, rollingAvgUsd, 3.0) === "pause") {
            ...
            if (advancedPastUnit) {
              ...
              ctx.ui.notify(
                `Unit ${s.currentUnit.id} cost spike detected (${formatCost(unitCostUsd)} vs avg ${formatCost(rollingAvgUsd)}) after state advanced; continuing closeout.`,
                "warning",
              );
              return "continue";
            }
            ctx.ui.notify(
              `Unit ${s.currentUnit.id} cost spike detected (${formatCost(unitCostUsd)} vs avg ${formatCost(rollingAvgUsd)}) — pausing auto-mode.`,
              "error",
            );
            await pauseAuto(ctx, pi);
            return "dispatched";
