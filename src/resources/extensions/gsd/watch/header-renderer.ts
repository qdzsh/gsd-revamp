...
export function gatherHeaderData(projectRoot: string): HeaderData {
  const rawModel = readModelFromPreferences();
  const model = formatDisplayModel(rawModel) || "default";
  const provider = deriveProvider(rawModel);
...
export function renderHeaderLines(data: HeaderData, width: number): string[] {
  ...
  for (let i = 0; i < logoLines.length; i++) {
    const logo = padVisible(color(logoLines[i], colors.logo), logoWidth);
