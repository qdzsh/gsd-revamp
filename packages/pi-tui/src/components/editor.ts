...
export class Editor implements Component, Focusable {
	...
	render(width: number): string[] {
		...
		const inputPrefix = visibleWidth(this.inputPrefix) < contentWidth ? this.inputPrefix : "";
		const inputPrefixWidth = visibleWidth(inputPrefix);
		const textWidth = Math.max(1, contentWidth - inputPrefixWidth);
		...
		for (const [visibleIndex, layoutLine] of visibleLines.entries()) {
			...
			const linePrefix = absoluteLineIndex === 0 ? inputPrefix : " ".repeat(inputPrefixWidth);
		...
		if (this.autocompleteState && this.autocompleteList) {
			const autocompleteResult = this.autocompleteList.render(textWidth);
			...
			for (const line of autocompleteResult) {
				...
				result.push(`${leftPadding}${" ".repeat(inputPrefixWidth)}${line}${linePadding}${rightPadding}`);
			}
			for (let i = autocompleteResult.length; i < reservedRows; i++) {
				result.push(`${leftPadding}${" ".repeat(inputPrefixWidth)}${" ".repeat(textWidth)}${rightPadding}`);
			}
		}
