...
export function mergeMilestoneToMain(
  ...
  try {
    ...
    if (status) {
      ...
      try {
        autoCommitted = autoCommitDirtyState(originalBasePath_);
        if (autoCommitted) {
          logWarning("worktree", `auto-committed dirty state on integration branch before squash merge for ${milestoneId}`);
        }
      } catch (autoCommitErr) {
        logWarning("worktree", `auto-commit before merge failed; falling back to stash: ${autoCommitErr instanceof Error ? autoCommitErr.message : String(autoCommitErr)}`);
      }
      if (!autoCommitted) {
