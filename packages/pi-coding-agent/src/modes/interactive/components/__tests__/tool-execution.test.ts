...
describe("ToolExecutionComponent", () => {
	test("renders framed header with running status while tool is partial", () => {
		...
		assert.match(rendered, /Esc to cancel/);
	});

	test("running status remains stable between renders without a tool update", async () => {
		const component = new ToolExecutionComponent(
			"mcp__demo__do_thing",
			{ ok: true },
			{},
	...
	test("renders framed header with failed status for failed tool result", () => {
		...
		assert.doesNotMatch(rendered, /Esc to cancel/);
	...
	test("compact tool cards use a status marker and a separate hidden-output hint", () => {
		const rendered = renderToolCollapsed(
			"read",
		...
		assert.match(rendered, /^\s+✓ Read src\/Inspector\.tsx\s+success · \d+(ms|s)\s*$/m);
		assert.match(rendered, /^\s+output hidden · ctrl\+o expand\s*$/m);
	...
	test("renders phase-based summaries for rolled-up tool executions", () => {
		...
		assert.match(rendered, /✓ Setup \/ shell · 6 commands\s+success · 12ms/);
	...
	test("rolled-up tool summaries can expand retained tool output after completion", () => {
		const first = new ToolExecutionComponent("read", { path: "src/a.ts" }, {}, undefined, { requestRender() {} } as any);
		first.updateResult({ content: [{ type: "text", text: "alpha source" }], isError: false });
		first.setExpanded(false);
		const second = new ToolExecutionComponent("read", { path: "src/b.ts" }, {}, undefined, { requestRender() {} } as any);
		second.updateResult({ content: [{ type: "text", text: "beta source" }], isError: false });
		second.setExpanded(false);

		const summary = new ToolPhaseSummaryComponent(
			[{ label: "Context reads", count: 2, durationMs: 4, targets: ["src/a.ts", "src/b.ts"] }],
			[first, second],
		);

		const collapsed = stripAnsi(summary.render(120).join("\n"));
		assert.match(collapsed, /Context reads · 2 files/);
		assert.doesNotMatch(collapsed, /alpha source/);

		summary.setExpanded(true);
		const expanded = stripAnsi(summary.render(120).join("\n"));
		assert.match(expanded, /alpha source/);
		assert.match(expanded, /beta source/);
	});
