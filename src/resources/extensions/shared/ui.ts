...
export interface UI {
	...
	// ── Special slots ─────────────────────────────────────────────────────────
	/** "Type something." / "Done" slot — selected state */
	slotSelected(label: string, description: string, isCommitted?: boolean): string[];
	/** "Type something." / "Done" slot — unselected state */
	...
	questionTabs(headers: string[], currentIndex: number, answeredIndices: Set<number>, opts?: { includeSubmit?: boolean }): string[];
...
export function makeUI(theme: Theme, width: number): UI {
	...
	return {
		...
		questionTabs: (headers, currentIndex, answeredIndices, opts = {}) => {
			...
			if (opts.includeSubmit) {
				const submitIndex = headers.length;
				const isCurrent = currentIndex === submitIndex;
				const label = ` ${GLYPH.check} Submit `;
				parts.push(isCurrent
					? theme.bg("selectedBg", theme.fg("text", label))
