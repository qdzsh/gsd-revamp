...
async function resolveGraphApi(): Promise<GraphApi> {
  ...
  try {
    const imported = await import("@gsd-revamp/mcp-server");
    if (isGraphApi(imported)) {
      cachedGraphApi = imported;
      return cachedGraphApi;
    }
    logWarning("prompt", "@gsd-revamp/mcp-server graph exports unavailable; using local graph fallback");
  } catch {
 ...
 *
 * Returns null when:
 * - @gsd-revamp/mcp-server fails to import
