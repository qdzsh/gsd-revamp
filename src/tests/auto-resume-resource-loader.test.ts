...
test("dev CLI wrapper passes itself as every child-process CLI entrypoint", () => {
  ...
  const devCliPath = join(root, "scripts", "dev-cli.js");
  const resolveTsPath = join(root, "src", "resources", "extensions", "gsd", "tests", "resolve-ts.mjs");
  const srcLoaderPath = join(root, "src", "loader.ts");
  const preflight = devCli.buildWorkspaceBuildPreflight({ root });
  ...
  const env = devCli.buildDevCliChildEnv({ PATH: "/usr/bin" }, devCliPath);
  assert.equal(env.PATH, "/usr/bin");
  assert.equal(env.GSD_DEV_CLI_PATH, devCliPath);
  assert.equal(env.GSD_CLI_PATH, devCliPath);
  assert.equal(env.GSD_BIN_PATH, devCliPath);
