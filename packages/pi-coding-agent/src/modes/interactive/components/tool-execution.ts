...
function shortenTargetForDisplay(target: string): string {
	const suffix = target.match(/:\d+(?:-\d+)?$/)?.[0] ?? "";
	const base = suffix ? target.slice(0, -suffix.length) : target;
	return `${shortenPath(base)}${suffix}`;
}
...
function summarizePhaseTargets(phase: ToolExecutionPhase, width: number): string | undefined {
	const phaseTargets = uniqueTargets(phase.targets).map(shortenTargetForDisplay);
...
export class ToolExecutionComponent extends Container {
	...
	private statusUpdatedAt = this.startedAt;
	...
	updateResult(
		...
		this.statusUpdatedAt = this.endedAt ?? Date.now();
	...
	markHistoricalNoResult(): void {
		...
		this.endedAt = this.endedAt ?? Date.now();
		this.statusUpdatedAt = this.endedAt;
		this.result = {
			content: [],
			isError: false,
		};
	...
	override render(width: number): string[] {
		...
		const elapsed = formatElapsed((this.endedAt ?? this.statusUpdatedAt) - this.startedAt);
		const statusWord = this.isPartial || !this.result ? "running" : this.result.isError ? "failed" : "success";
		const interruptHint = this.isPartial || !this.result ? " · Esc to cancel" : "";
		const frameStatus = `${statusWord} · ${elapsed}${interruptHint}`;
...
export class ToolPhaseSummaryComponent extends Container {
	...
	constructor(
		private readonly phases: ToolExecutionPhase[],
		private readonly rolledUpComponents: ToolExecutionComponent[] = [],
	) {
		super();
	}
	...
	getRolledUpComponents(): ToolExecutionComponent[] {
		return [...this.rolledUpComponents];
	}

	setExpanded(expanded: boolean): void {
		this.expanded = expanded;
		for (const component of this.rolledUpComponents) {
	...
	override render(width: number): string[] {
		const frameWidth = Math.max(20, width);
		if (this.expanded && this.rolledUpComponents.length > 0) {
			return this.rolledUpComponents.flatMap((component) => component.render(frameWidth));
		}
		...
		const rows = this.phases.flatMap((phase) => {
			...
			const summaryRow = renderToolStatusRows(theme.fg("borderAccent", left), frameWidth, {
				status: right,
				tone: "success",
			});
			const targetRow = summarizePhaseTargets(phase, Math.max(1, frameWidth - 2));
			return targetRow ? [...summaryRow, theme.fg("muted", `  ${targetRow}`)] : summaryRow;
		});

		return ["", ...renderToolBlock(rows, frameWidth, "success")];
	}
}
