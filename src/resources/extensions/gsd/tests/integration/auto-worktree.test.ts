...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "auto-wt-test-")));
  run("git init", dir);
...
describe("auto-worktree lifecycle", () => {
  ...
  test("symlink-resolved auto worktree is detected after module state reset", () => {
    ...
    const fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), "auto-wt-home-")));
  ...
  test("#2482: throws GSDError when repo has no commits", () => {
    ...
    tempDir = realpathSync.native(mkdtempSync(join(tmpdir(), "auto-wt-empty-")));
