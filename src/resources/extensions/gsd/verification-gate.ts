...
export interface VerifyNormalizationChange {
  before: string;
  after: string;
}

export interface VerifyNormalizationResult {
  verify: string;
  changes: VerifyNormalizationChange[];
}
...
function normalizeVerifyCommand(cmd: string): string {
  let out = (typeof cmd === "string" ? cmd : "").trim();
  if (!out) return out;

  out = out.replace(/\s*\|\|\s*(true|:)\s*$/u, "");

  const catGrep = out.match(/^cat\s+(\S+)\s*\|\s*grep((?:\s+-[A-Za-z]+)*)\s+(.+)$/u);
  if (catGrep) {
    const file = catGrep[1];
    const flags = catGrep[2];
    const rest = catGrep[3].trim();
    if (rest.length > 0 && !rest.includes("|")) {
      out = (`grep${flags} ${rest} ${file}`).trim();
    }
  }

  const catWc = out.match(/^cat\s+(\S+)\s*\|\s*wc(\s+-[A-Za-z]+)?$/u);
  if (catWc) {
    const file = catWc[1];
    const flag = catWc[2] ?? "";
    out = (`wc${flag} ${file}`).trim();
  }

  return out;
}

export function normalizeVerifyString(verify: string): VerifyNormalizationResult {
  const changes: VerifyNormalizationChange[] = [];
  if (typeof verify !== "string" || !verify.trim()) {
    return { verify: typeof verify === "string" ? verify : "", changes };
  }
  ...
  for (const segment of segments) {
    const before = segment.trim();
    if (!before) continue;
    const after = normalizeVerifyCommand(before);
    if (after !== before) changes.push({ before, after });
    rewritten.push(after);
  }

  return { verify: rewritten.join(" && "), changes };
}
...
export function discoverCommands(options: DiscoverCommandsOptions): DiscoveredCommands {
  ...
  if (taskPlanVerify) {
    ...
    const candidates = normalizeVerifyString(taskPlanVerify).verify
      .split(/&&|\r?\n/)
      .map(c => c.trim())
      .filter(Boolean);
    for (const candidate of candidates) {
      ...
      if (validation.ok) {
        commands.push(validation.normalized ?? candidate);
 ...
 */
export function validateVerificationCommand(cmd: string): { ok: true; normalized?: string } | { ok: false; reason: string } {
  if (hasUnsafeShellSyntax(cmd)) {
    const rewritten = normalizeVerifyCommand(cmd);
    if (rewritten !== cmd && !hasUnsafeShellSyntax(rewritten) && isLikelyCommand(rewritten)) {
      return { ok: true, normalized: rewritten };
    }
  }
  if (hasUnsafeShellSyntax(cmd)) {
