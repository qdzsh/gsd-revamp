...
describe('repo-identity-worktree', () => {
  ...
  before(() => {
    base = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-repo-identity-")));
    stateDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-state-")));
...
test('ensureGsdSymlink refreshes repo-meta gitRoot after repo move with fixed project id', () => {
      const moveRepo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-repo-identity-move-")));
...
test('isInheritedRepo detects subdirectory of parent repo without .gsd (#1639)', () => {
      const parentRepo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-inherited-parent-")));
      ...
      const standaloneRepo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-inherited-standalone-")));
...
test('subdirectory of parent repo gets unique identity after git init (#1639)', () => {
      const parentRepo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-identity-parent-")));
      run("git init -b main", parentRepo);
      run('git config user.name "Pi Test"', parentRepo);
      run('git config user.email "pi@example.com"', parentRepo);
...
test('ensureGsdSymlink from subdirectory does not create .gsd in subdir when git-root .gsd exists (#2380)', () => {
    const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-subdir-symlink-")));
...
test('ensureGsdSymlink prefers marker directory when marker/computed identities diverge and both have state (#5685)', () => {
    const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-split-brain-")));
    run("git init -b main", repo);
    run('git config user.name "Pi Test"', repo);
    run('git config user.email "pi@example.com"', repo);
