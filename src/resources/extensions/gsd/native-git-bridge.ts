...
function resolveGitExecutable(env: NodeJS.ProcessEnv = GIT_NO_PROMPT_ENV): string {
...
function execGitFile(args: string[], options: ExecFileSyncOptionsWithStringEncoding): string {
  const env = options.env as NodeJS.ProcessEnv | undefined;
  const git = resolveGitExecutable(env);
  if (process.platform === "win32" && /\.(cmd|bat)$/i.test(git)) {
    return execFileSync(process.env.ComSpec ?? "cmd.exe", ["/d", "/c", git, ...args], options);
  }
...
export function nativeMergeSquash(basePath: string, branch: string): GitMergeResult {
  ...
  try {
    execGitFile(["merge", "--squash", branch], {
      cwd: basePath,
      stdio: ["ignore", "pipe", "pipe"],
      encoding: "utf-8",
      env: GIT_NO_PROMPT_ENV,
