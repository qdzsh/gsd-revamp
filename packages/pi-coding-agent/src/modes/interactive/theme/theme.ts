...
import type { ColorValue, ThemeJson } from "./theme-schema.js";
...
const STANDARD_THEME_NAME = "dark";
...
export class Theme {
	...
	fg(color: ThemeColor, text: string): string {
		if (color === "dim") return chalk.dim(text);
...
function getBuiltinThemes(): Record<string, ThemeJson> {
	return { [STANDARD_THEME_NAME]: builtinThemes[STANDARD_THEME_NAME] };
}

function withSemanticColorDefaults(colors: ThemeJson["colors"]): ThemeJson["colors"] {
	return {
		...colors,
		logo: colors.logo ?? colors.success,
...
function loadThemeJson(name: string): ThemeJson {
	...
	throw new Error(`Theme switching is disabled; only "${STANDARD_THEME_NAME}" is available.`);
}
...
export function initTheme(themeName?: string, enableWatcher: boolean = false): void {
	...
	try {
		...
		setGlobalTheme(loadTheme(STANDARD_THEME_NAME));
	}
}
...
export function getResolvedThemeColors(themeName?: string): Record<string, string> {
	void themeName;
	const themeJson = loadThemeJson(STANDARD_THEME_NAME);
