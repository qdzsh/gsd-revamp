...
test("parseArgs accepts json, root, command, compare, and output options", () => {
  ...
  assert.equal(opts.root, resolve("/tmp/example"));
  assert.deepEqual(opts.commands, [{ label: "noop", command: "node -e 1" }]);
  assert.equal(opts.compare, resolve("/tmp/before.json"));
  assert.equal(opts.output, resolve("/tmp/after.json"));
});
...
test("collectBaseline returns the phase-zero report shape", async () => {
  const root = await makeFixtureRoot();
  await writeFile(join(root, "README.md"), "# GSD\n");
  await mkdir(join(root, "docs/user-docs"), { recursive: true });
  await writeFile(join(root, "docs/user-docs/auto-mode.md"), "# Auto mode\n");
...
test("collectProcessMetrics reports Phase 7 process dashboard fields", async () => {
  ...
  await writeProcessMetricFixtures(root);
  await writeFile(
    join(root, "src/resources/extensions/gsd/docs/conflict.md"),
  ...
  assert.deepEqual(metrics.docsConflictFiles, ["src/resources/extensions/gsd/docs/conflict.md"]);
...
test("countLegacyContractImports ignores rpc-client implementation types", () => {
  assert.equal(
    countLegacyContractImports(`
      import type { RpcClient } from "@gsd-revamp/rpc-client";
      import type { SdkAgentEvent, RpcCostUpdateEvent } from "@gsd-revamp/rpc-client";
    `),
    2,
  );
  assert.equal(
    countLegacyContractImports('import type { RpcClientOptions } from "@gsd-revamp/rpc-client";'),
    0,
  );
});
...
async function writeContractsSurfaceFixtures(root: string): Promise<void> {
  ...
  for (const file of files) {
    ...
    await writeFile(join(root, file), 'import type { RpcCommand } from "@gsd-revamp/contracts";\n');
  }
}
