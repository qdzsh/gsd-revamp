...
describe('shared-wal', async () => {
  ...
  {
    const projectRoot = resolve('/home/user/myproject');
  ...
  console.log('\n=== shared-wal: resolve forward-slash path ===');
  {
    ...
    const result = resolveProjectRootDbPath(join(projectRoot, '.gsd', 'worktrees', 'M001'));
    assert.deepStrictEqual(result, join(projectRoot, '.gsd', 'gsd.db'),
  ...
  console.log('\n=== shared-wal: resolve external-state worktree path (#2952) ===');
  {
    ...
    const stateRoot = resolve('/home/user/.gsd/projects/a1b2c3d4');
  ...
  {
    const stateRoot = resolve('/home/user/.gsd/projects/deadbeef42');
  ...
  {
    const stateRoot = resolve('/Users/dev/.gsd/projects/cafe0123');
    const result = resolveProjectRootDbPath(join(stateRoot, 'worktrees', 'M001'));
    assert.deepStrictEqual(result, join(stateRoot, 'gsd.db'),
