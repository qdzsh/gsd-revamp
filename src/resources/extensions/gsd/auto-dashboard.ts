  ...
  ctx.ui.setWidget("gsd-progress", (tui, theme) => {
    ...
    return {
      render(width: number): string[] {
        ...
        const headerLeft = [
          `${pad}${spinner} ${theme.fg("logo", theme.bold("GSD"))}`,
  ...
  ctx.ui.setWidget(widgetKey, (_tui, theme) => ({
    render(width: number): string[] {
      ...
      const addSection = (label: string, value: string | null | undefined, indent = ""): void => {
        ...
        if (!clean) return;
        add(`${indent}${theme.fg("logo", label)} ${theme.fg("text", truncateToWidth(clean, contentWidth - indent.length - label.length - 1, "…"))}`);
      };
      const addList = (label: string, values: string[] | undefined, limit: number, indent = ""): void => {
        ...
        add(`${indent}${theme.fg("logo", label)} ${theme.fg("text", truncateToWidth(shown.join("; ") + more, contentWidth - indent.length - label.length - 1, "…"))}`);
      };
      ...
      const heading = snapshot.allMilestonesComplete
        ...
        : snapshot.milestoneId
          ...
          : "Milestone roll-up";
      lines.push(rightAlign(`${pad}${theme.fg("logo", theme.bold(heading))}`, elapsed ? theme.fg("dim", elapsed) : "", width));
      ...
      if (changed.length > 0) {
        lines.push("");
        add(theme.fg("logo", "What changed"));
      ...
      if (verification.length > 0 || (snapshot.keyFiles?.length ?? 0) > 0) {
        lines.push("");
        add(theme.fg("logo", "Verification"));
      ...
      if (snapshot.totalCost > 0) stats.push(theme.fg("warning", formatCost(snapshot.totalCost)));
      ...
      if (stats.length > 0) {
        add(`${theme.fg("logo", "Run totals")} ${stats.join(theme.fg("dim", " · "))}`);
      }
...
export function setAutoOutcomeWidget(
  ...
  ctx.ui.setWidget("gsd-outcome", (_tui, theme) => ({
    render(width: number): string[] {
      ...
      const heading = `${theme.fg(color, icon)} ${theme.fg("logo", theme.bold("GSD"))} ${theme.fg("text", snapshot.title)}`;
