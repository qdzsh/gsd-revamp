...
function getLinkablePackages() {
	...
	for (const dir of entries) {
		...
		if (gsd.scope !== '@gsd' && gsd.scope !== '@gsd-revamp') {
			throw new Error(
				`${pkgJsonPath}: "gsd.scope" must be "@gsd" or "@gsd-revamp" (got "${gsd.scope}").`
			)
		}
...
/** Returns only packages in the `@gsd` scope (excludes `@gsd-revamp`). */
