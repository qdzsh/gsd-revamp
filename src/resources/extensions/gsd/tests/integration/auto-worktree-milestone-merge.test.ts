...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-ms-merge-test-")));
  run("git init", dir);
...
function createTempRepoWithExternalGsd(): { repo: string; externalState: string } {
  ...
  const repo = realpathSync.native(mkdtempSync(join(realTmp, "wt-ms-merge-ext-test-")));
  const externalState = realpathSync.native(mkdtempSync(join(realTmp, "wt-ms-merge-ext-state-")));
...
describe("auto-worktree-milestone-merge", { timeout: 300_000 }, () => {
  ...
  test("auto-push with bare remote", () => {
    ...
    const bareDir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-ms-bare-")));
  ...
  test("master-branch repo — no META.json, no prefs (#1668)", () => {
    const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "wt-ms-master-test-")));
    tempDirs.push(dir);
