...
export async function checkGitHealth(
  ...
  try {
    ...
    if (existsSync(wtDir)) {
      ...
      const nativeEntries = nativeWorktreeList(basePath);
      const registeredPaths = new Set(nativeEntries.map(entry => normalizePath(entry.path)));
      const registeredNames = new Set(
        nativeEntries
          .map(entry => entry.branch.startsWith("worktree/")
      ...
      for (const entry of readdirSync(wtDir)) {
        ...
        const normalizedEntryName = process.platform === "win32" ? entry.toLowerCase() : entry;
        if (!registeredPaths.has(normalizedFullPath) && !registeredNames.has(normalizedEntryName)) {
