...
import { existsSync, mkdtempSync, mkdirSync, writeFileSync, rmSync, realpathSync, chmodSync } from "node:fs";
...
function withEnv(vars: Record<string, string | undefined>, fn: () => void): void {
  ...
  const tempHome = vars.GSD_HOME ?? vars.HOME ?? vars.USERPROFILE;
  ...
  if (tempHome && !("GSD_HOME" in vars)) {
    saved.GSD_HOME = process.env.GSD_HOME;
    process.env.GSD_HOME = join(tempHome, ".gsd");
  }
  if (tempHome && !("USERPROFILE" in vars)) {
    saved.USERPROFILE = process.env.USERPROFILE;
    process.env.USERPROFILE = tempHome;
  }
  try {
    if (tempHome) {
      const gsdDir = process.env.GSD_HOME ?? join(tempHome, ".gsd");
...
function removeTempRoot(path: string): void {
  const current = process.cwd().replaceAll("\\", "/").toLowerCase();
  const target = path.replaceAll("\\", "/").replace(/\/+$/, "").toLowerCase();
...
test("runProviderChecks detects Anthropic key from ANTHROPIC_API_KEY env var", () => {
  ...
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-env-test-")));
  withEnv({ ANTHROPIC_API_KEY: "sk-ant-test-key", ANTHROPIC_OAUTH_TOKEN: undefined, HOME: tmpHome }, () => {
    try {
      ...
      removeTempRoot(tmpHome);
    }
  });
});

test("runProviderChecks returns error for Anthropic when no key present", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-test-")));
  withEnv({
  ...
  withEnv(
    ...
    () => {
      ...
      try {
        ...
        removeTempRoot(process.env.HOME!);
...
test("runProviderChecks detects custom provider keys from models.json", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-custom-home-")));
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-custom-repo-")));
  const agentDir = join(tmpHome, ".gsd", "agent");
  mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks reports missing custom provider key without models.json apiKey", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-custom-missing-home-")));
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-custom-missing-repo-")));
  mkdirSync(join(repo, ".gsd"), { recursive: true });
...
test("runProviderChecks reports ok for Anthropic when GitHub Copilot env var is set", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-copilot-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
...
test("runProviderChecks reports ok for Anthropic via GITHUB_TOKEN cross-provider routing", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-ghtoken-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
    ANTHROPIC_OAUTH_TOKEN: undefined,
    COPILOT_GITHUB_TOKEN: undefined,
...
test("runProviderChecks detects ANTHROPIC_OAUTH_TOKEN as valid Anthropic auth", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-oauth-test-")));
  withEnv({
    ANTHROPIC_API_KEY: undefined,
...
test("runProviderChecks reports ok via Copilot auth.json for Anthropic", () => {
  withEnv({
    ...
    const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-copilot-auth-test-")));
    const agentDir = join(tmpHome, ".gsd", "agent");
    mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks uses provider-qualified anthropic-vertex model IDs", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-vertex-prefix-home-")));
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-vertex-prefix-repo-")));
  mkdirSync(join(repo, ".gsd"), { recursive: true });
  writeFileSync(
    join(repo, ".gsd", "PREFERENCES.md"),
    [
...
test("runProviderChecks uses object provider field for anthropic-vertex models", () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-vertex-provider-home-")));
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-vertex-provider-repo-")));
  mkdirSync(join(repo, ".gsd"), { recursive: true });
  writeFileSync(
    join(repo, ".gsd", "PREFERENCES.md"),
    [
...
test("runProviderChecks reports ok for OpenAI via openai-codex auth.json (#2922)", () => {
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-codex-repo-")));
  mkdirSync(join(repo, ".gsd"), { recursive: true });
  writeFileSync(
    join(repo, ".gsd", "PREFERENCES.md"),
    [
  ...
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-codex-home-")));
  const agentDir = join(tmpHome, ".gsd", "agent");
  mkdirSync(agentDir, { recursive: true });
...
test("runProviderChecks reports ok for claude-code without any API key", () => {
  const repo = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-cc-repo-")));
  mkdirSync(join(repo, ".gsd"), { recursive: true });
  writeFileSync(
    join(repo, ".gsd", "PREFERENCES.md"),
    [
  ...
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-cc-home-")));
...
test("runProviderChecks reports ok for Anthropic via claude-code binary in PATH", () => {
  ...
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-cc-route-home-")));
...
test("runProviderChecks detects claude.cmd in PATH on Windows (#4503)", { skip: process.platform !== "win32" }, () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-cc-win-route-home-")));
  const binDir = join(tmpHome, "bin");
  mkdirSync(binDir, { recursive: true });
...
test("runProviderChecks detects claude.exe in PATH on Windows (#4548)", { skip: process.platform !== "win32" }, () => {
  const tmpHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-providers-cc-exe-home-")));
  const binDir = join(tmpHome, "bin");
  mkdirSync(binDir, { recursive: true });
