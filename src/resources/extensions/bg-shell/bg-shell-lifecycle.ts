...
import { formatDisplayModel, formatUsageDisplay, formatUsageContext } from "../shared/usage-display.js";
		...
		state.latestCtx.ui.setFooter((tui, th, footerData) => {
			...
			return {
				render(width: number): string[] {
					...
					const contextDisplay = formatUsageContext(usageStats);
					...
					const usageWithoutContext = formatUsageDisplay(usageStats, { includeContext: false });
					let statsLeft = usageWithoutContext && contextStr
						? `${usageWithoutContext} · ${contextStr}`
						: usageWithoutContext || contextStr;
					...
					const modelName = formatDisplayModel(ctx?.model?.id) || "no-model";
