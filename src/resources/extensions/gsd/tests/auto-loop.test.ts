...
test("autoLoop preserves stuck recovery counter when dispatch recovery continues", async () => {
  ...
  const ctx = makeMockCtx();
  const pi = makeMockPi();
  const basePath = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-stuck-counter-reset-")));
...
test("autoLoop skips provider dispatch when execute-task is already complete in DB", async () => {
  ...
  ctx.ui.setWidget = () => {};
  const pi = makeMockPi();
  const basePath = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-already-complete-dispatch-")));
  mkdirSync(join(basePath, ".gsd"), { recursive: true });
