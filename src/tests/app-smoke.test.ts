...
test("gsd update and upgrade bypass the managed-resource-mismatch gate; other commands trigger it", async (t) => {
  ...
  writeFileSync(
    ...
    JSON.stringify({ gsdVersion: futureVersion, packageName: "gsd-revamp", syncedAt: Date.now() }),
  );
...
test("managed resource skew ignores legacy manifests from the old npm package", async (t) => {
  ...
  assert.strictEqual(
    ...
    null,
    "old unscoped gsd-pi resource stamps must not block gsd-revamp startup",
  );
});
...
test("managed resource skew ignores manifests stamped by a different package", async (t) => {
  ...
  assert.strictEqual(
    getNewerManagedResourceVersion(fakeAgentDir, "1.0.1"),
    null,
    "old gsd-pi resource stamps must be refreshed instead of treated as newer gsd-revamp resources",
  );
});
...
test("managed resource skew ignores dev/build suffixes on the same release line", async (t) => {
  ...
  writeFileSync(
    join(fakeAgentDir, "managed-resources.json"),
    JSON.stringify({ gsdVersion: "2.78.1", packageName: "gsd-revamp", syncedAt: Date.now() }),
  );
