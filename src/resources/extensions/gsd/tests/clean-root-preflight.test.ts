...
function createTempRepo(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-preflight-test-")));
  run("git init", dir);
...
test("preflightCleanRoot — skips stash on Windows reserved untracked name", { skip: process.platform === "win32" }, () => {
