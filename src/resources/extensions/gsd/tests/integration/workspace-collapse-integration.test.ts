...
function makeProjectDir(): string {
  const dir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-collapse-int-")));
...
describe("workspace-collapse integration: Test 1 — cwd-drift path agreement", () => {
  ...
  beforeEach(() => {
    ...
    otherDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-cwd-drift-other-")));
    _clearGsdRootCache();
  });
...
describe("workspace-collapse integration: Test 3 — write-gate snapshot survives cwd drift", () => {
  ...
  beforeEach(() => {
    projectDir = makeProjectDir();
    otherDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-wg-other-")));
...
describe("workspace-collapse integration: Test 5 — gsdRootCache normalization deduplicates trailing-slash inputs", () => {
  ...
  beforeEach(() => {
    projectDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-cache-int-")));
    mkdirSync(join(projectDir, ".gsd"), { recursive: true });

    fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-cache-int-home-")));
