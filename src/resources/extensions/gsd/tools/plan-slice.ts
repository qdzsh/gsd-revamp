...
import { checkFilePathConsistency, checkTaskOrdering, checkVerificationCommands } from "../pre-execution-checks.js";
import { normalizeVerifyString } from "../verification-gate.js";
...
function validateTasks(value: unknown): PlanSliceTaskInput[] {
  ...
  return value.map((entry, index) => {
    ...
    return {
      ...
      verify: normalizeVerifyString(verify).verify,
...
function validateTaskPathsBeforePersist(params: PlanSliceParams, basePath: string, defaultTargets: string[]): string | null {
  ...
  const checks = [
    ...checkVerificationCommands(taskRows),
