...
export class ParallelMonitorOverlay {
  ...
  render(width: number): string[] {
    ...
    if (this.workers.length === 0) {
      ...
      for (const wk of this.workers) {
        ...
        lines.push(
          ...
          `cost ${t.bold(formatCost(wk.cost))}  ` +
    ...
    if (allDone) {
      ...
      for (const wk of this.workers) {
        lines.push(
          `  ${wk.mid}  ${formatCost(wk.cost)}  │  ${wk.doneSlices}/${wk.totalSlices} slices  ` +
          `${wk.doneTasks}/${wk.totalTasks} tasks  │  ${formatDuration(wk.elapsed)}`,
        );
      }
      lines.push(`  ${t.bold("Total: " + formatCost(this.workers.reduce((s, wk) => s + wk.cost, 0)))}`);
    }
