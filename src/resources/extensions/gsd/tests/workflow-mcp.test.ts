...
test("detectWorkflowMcpLaunchConfig prefers explicit env override", () => {
  ...
  assert.equal(launch?.env?.GSD_WORKFLOW_PROJECT_ROOT, projectRoot);
...
test("buildWorkflowMcpServers mirrors explicit launch config", () => {
  ...
  assert.equal((servers?.["gsd-workflow"]?.env as Record<string, string> | undefined)?.GSD_WORKFLOW_PROJECT_ROOT, projectRoot);
