...
Then **offer GitHub issue creation**: "Would you like me to create a GitHub issue for this on qdzsh/gsd-revamp?"

**CRITICAL:** The `github_issues` tool targets only the current user's repository and has no `repo` parameter. Use `gh issue create --repo qdzsh/gsd-revamp` via `bash`. Do NOT use the `github_issues` tool.
...
ISSUE_URL=$(gh issue create --repo qdzsh/gsd-revamp \
  --title "..." \
