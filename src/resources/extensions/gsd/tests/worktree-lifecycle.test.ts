...
function makeGitRepoBase(opts?: {
  ...
  const base = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-lifecycle-git-")));
...
function makeDbBase(): string {
  const base = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-lifecycle-")));
  mkdirSync(join(base, ".gsd"), { recursive: true });
  return base;
}
...
test("enterMilestone returns ok:true mode:worktree on successful create", (t) => {
  ...
  if (result.ok) {
    ...
    assert.ok(
      result.path.replaceAll("\\", "/").endsWith("/.gsd/worktrees/M001"),
      `expected path to end with /.gsd/worktrees/M001, got ${result.path}`,
    );
  }
  assert.ok(
    s.basePath.replaceAll("\\", "/").endsWith("/.gsd/worktrees/M001"),
...
test("enterMilestone returns ok:false reason:lease-conflict when another worker holds the lease", (t) => {
  ...
  _getAdapter()!.prepare("UPDATE workers SET pid = pid + 1 WHERE worker_id = :worker_id").run({ ":worker_id": contender });
...
test("resumeFromPausedSession adopts the persisted worktree path when it is a git worktree", (t) => {
  const wtDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-resume-test-")));
...
test("resumeFromPausedSession falls back to base when persisted directory is not a git worktree", (t) => {
  const wtDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-resume-stub-")));
