...
export function resolveGsdRoot(projectDir: string): string {
  ...
  const homeKey = comparePath(home);
  ...
  try {
    ...
    if (comparePath(gitRoot) !== homeKey && existsSync(gitGsd) && statSync(gitGsd).isDirectory()) {
  ...
  // Walk up from projectDir only inside a known git worktree. This preserves
  // subdirectory support without accidentally adopting unrelated ancestor
  // state such as %TEMP%/.gsd or the user's global ~/.gsd.
  if (gitRoot) {
    let dir = resolved;
    const gitRootKey = comparePath(gitRoot);
    while (dir !== dirname(dir)) {
      const dirKey = comparePath(dir);
      if (dirKey === homeKey) break;
      const candidate = join(dir, '.gsd');
      if (existsSync(candidate) && statSync(candidate).isDirectory()) {
        rememberGsdRoot(resolved, candidate);
        return candidate;
      }
      if (dirKey === gitRootKey) break;
      dir = dirname(dir);
    }
  }
