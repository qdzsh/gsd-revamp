...
import { formatDisplayModel } from "../../shared/usage-display.js";
...
const REQUIRED_CLOSEOUT_TOOL_NAMES = new Set(["gsd_task_complete", "gsd_complete_task"]);

function requiredCloseoutToolName(block: unknown): string {
  if (!isObjectRecord(block)) return "";
  const name = block.name ?? block.toolName ?? block.tool_name;
  return typeof name === "string" ? name : "";
}

function hasRequiredCloseoutTool(event: AgentEndEvent): boolean {
  const messages = Array.isArray(event.messages) ? event.messages : [];
  for (const message of messages) {
    if (REQUIRED_CLOSEOUT_TOOL_NAMES.has(requiredCloseoutToolName(message))) {
      return true;
    }
    const content = isObjectRecord(message) ? message.content : undefined;
    const blocks = Array.isArray(content) ? content : [];
    for (const block of blocks) {
      if (!isObjectRecord(block)) continue;
      if (
        (block.type === "toolCall" || block.type === "serverToolUse" || block.type === "tool_use") &&
        REQUIRED_CLOSEOUT_TOOL_NAMES.has(requiredCloseoutToolName(block))
      ) {
        return true;
      }
    }
...
function requiredCloseoutContext(): { unit: { id: string; startedAt?: unknown }; basePath?: string } | null {
  const dash = getAutoDashboardData();
  const unit = dash?.currentUnit;
  if (!dash?.active || !unit || unit.type !== "execute-task" || typeof unit.id !== "string") return null;
  return { unit, basePath: resolveAgentEndBasePath() };
}

function parseRequiredCloseoutUnit(unitId: string): { milestoneId: string; sliceId: string; taskId: string } | null {
  const match = unitId.match(/^([^/]+)\/(S\d+)\/(T\d+)$/);
  if (!match) return null;
  return { milestoneId: match[1], sliceId: match[2], taskId: match[3] };
}

function parseRequiredCloseoutStartedAt(startedAt: unknown): number {
...
function summaryFrontmatterMatches(text: string, unitParts: { milestoneId: string; sliceId: string; taskId: string }): boolean {
  const frontmatter = text.match(/^---\n([\s\S]*?)\n---/);
  if (!frontmatter) return false;
  const body = frontmatter[1];
  return new RegExp(`^id:\\s*${unitParts.taskId}\\s*$`, "m").test(body) &&
    new RegExp(`^parent:\\s*${unitParts.sliceId}\\s*$`, "m").test(body) &&
    new RegExp(`^milestone:\\s*${unitParts.milestoneId}\\s*$`, "m").test(body);
}

async function hasFreshTaskSummary(closeout: { unit: { id: string; startedAt?: unknown }; basePath?: string }): Promise<boolean> {
  const unitParts = parseRequiredCloseoutUnit(closeout.unit.id);
  const startedAtMs = parseRequiredCloseoutStartedAt(closeout.unit.startedAt);
  if (!unitParts || !closeout.basePath || !Number.isFinite(startedAtMs)) return false;
  try {
    ...
    const summaryPath = path.join(
      ...
      ".gsd",
      "milestones",
      unitParts.milestoneId,
      "slices",
      unitParts.sliceId,
      "tasks",
      `${unitParts.taskId}-SUMMARY.md`,
    );
    const [stat, text] = await Promise.all([fs.stat(summaryPath), fs.readFile(summaryPath, "utf8")]);
    return stat.mtimeMs + 1000 >= startedAtMs && summaryFrontmatterMatches(text, unitParts);
  } catch {
    return false;
  }
}

async function enforceRequiredCloseout(event: AgentEndEvent, ctx: ExtensionContext, pi: ExtensionAPI): Promise<boolean> {
  const closeout = requiredCloseoutContext();
  if (!closeout) return false;
  if (hasRequiredCloseoutTool(event) || await hasFreshTaskSummary(closeout)) return false;

  const message = `missing-required-closeout-tool: execute-task ${closeout.unit.id} ended without gsd_task_complete; rerun the task and only complete it after planned outputs exist and fresh verification has passed`;
...
export async function handleAgentEnd(
  ...
  if (isSessionSwitchAbortGraceActive() && isObjectRecord(lastMsg) && lastMsg.stopReason === "aborted" && !lastMsg.errorMessage) {
    // Generic aborted stopReason without explicit errorMessage arriving in
    // the session-switch grace window is a late abort tail from the prior turn.
    return;
  }
  ...
  if (isObjectRecord(lastMsg) && "stopReason" in lastMsg && lastMsg.stopReason === "error") {
          ...
          while (true) {
            ...
            if (candidate && !isModelBlocked(dash.basePath, candidate.provider, candidate.id)) {
              ...
              if (ok) {
                ...
                ctx.ui.notify(
                  `Switched to fallback ${formatDisplayModel(candidate.id)} after account entitlement rejection.`,
                  "warning",
                );
                pi.sendMessage(
        ...
        if (
          ...
          if (startModel) {
            ...
            if (ok) {
              ...
              ctx.ui.notify(
                `Restored auto-mode start model ${formatDisplayModel(startModel.id)} after entitlement rejection.`,
                "warning",
              );
              pi.sendMessage(
                { customType: "gsd-auto-timeout-recovery", content: "Continue execution.", display: false },
    ...
    // Cap rate-limit backoff for CLI-style providers (openai-codex)
          ...
          if (nextModelId) {
            ...
            if (modelToSet) {
              ...
              if (ok) {
                ...
                ctx.ui.notify(`Model error${errorDetail}. Switched to fallback: ${formatDisplayModel(nextModelId)} and resuming.`, "warning");
        ...
        if (ctx.model?.id !== sessionModel.id || ctx.model?.provider !== sessionModel.provider) {
          ...
          if (startModel) {
            ...
            if (ok) {
              ...
              retryState.networkRetryCount = 0;
              retryState.currentRetryModelId = undefined;
              ctx.ui.notify(`Model error${errorDetail}. Restored session model: ${formatDisplayModel(sessionModel.id)} and resuming.`, "warning");
              pi.sendMessage({ customType: "gsd-auto-timeout-recovery", content: "Continue execution.", display: false }, { triggerTurn: true });
              return;
            }
          }
  ...
  // ── Success path ─────────────────────────────────────────────────────────
  try {
    if (await enforceRequiredCloseout(event, ctx, pi)) {
      return;
    }
    resetRetryState(retryState);
