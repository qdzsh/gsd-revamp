...
function resolveCommand(runtime: ExecSandboxRequest["runtime"]): { cmd: string; args: string[] } {
  switch (runtime) {
    case "bash":
      return { cmd: resolveHostCommand("bash"), args: ["-c"] };
    case "node":
      return { cmd: process.execPath, args: ["-e"] };
    case "python":
      return { cmd: resolveHostCommand("python3"), args: ["-c"] };
  }
}

function resolveHostCommand(command: string): string {
  ...
  const candidates = process.platform === "win32"
    ? [command, ...extensions.map((ext) => `${command}${ext.startsWith(".") ? ext : `.${ext}`}`)]
    : [command];
...
export function runExecSandbox(
  ...
  return new Promise((resolveP) => {
    ...
    const timer = setTimeout(() => {
      ...
      } else if (process.platform === "win32" && child.pid != null) {
        try {
      ...
      setTimeout(() => finalize(null, "SIGKILL"), 250).unref?.();
    }, timeoutMs);
    timer.unref?.();
