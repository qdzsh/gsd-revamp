...
export async function refreshResumeResourcesAndDb(
  ...
  const { primeCache, invalidatePromptCache } = await importModule("./prompt-loader.js");
  if (typeof invalidatePromptCache === "function") {
    invalidatePromptCache("execute-task");
  }
  primeCache();
...
export async function stopAuto(
  ...
  try {
    ...
    // ── Step 10: Reset metrics, routing, hooks ──
    try {
      resetMetrics();
      resetRoutingHistory();
      resetHookState();
