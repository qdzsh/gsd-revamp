...
function findRepoRoot(start: string): string {
	...
	for (let i = 0; i < 10; i++) {
		try {
			...
			if (pkg.name === "gsd-revamp" && pkg.workspaces) return dir;
		} catch {
			// Keep walking.
		}
...
function assertContractsBefore(scriptName: string, laterCommand: string): void {
	...
	assert.ok(contractsIndex >= 0, `${scriptName} must build @gsd-revamp/contracts`);
	assert.ok(laterIndex >= 0, `${scriptName} must include ${laterCommand}`);
	assert.ok(
		contractsIndex < laterIndex,
		`${scriptName} must build @gsd-revamp/contracts before ${laterCommand}`,
	);
}
