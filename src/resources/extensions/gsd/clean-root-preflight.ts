...
export interface PostflightResult {
  ...
  message: string;
  stashRef?: string;
  resolution?: "applied" | "already-present-dropped" | "already-present-preserved" | "manual-recovery" | "missing-stash-clean-tree";
...
export function postflightPopStash(
  ...
  try {
    ...
    if (!stashRef) {
      ...
      let stashListReadable = true;
      try {
        execFileSync("git", ["stash", "list", "--format=%gd"], {
          cwd: basePath,
          stdio: ["ignore", "pipe", "pipe"],
          encoding: "utf-8",
          env: GIT_NO_PROMPT_ENV,
        ...
        stashListReadable = false;
        logWarning("preflight", `stash list verification failed before missing-stash clean-tree guard: ${err instanceof Error ? err.message : String(err)}`);
      }
      const cleanAfterMissingStash = stashListReadable ? isWorktreeClean(basePath) : null;
      if (stashListReadable && cleanAfterMissingStash === true) {
        const cleanMsg = `${msg} Working tree is clean; continuing auto-mode.`;
        logWarning("preflight", cleanMsg);
        notify(cleanMsg, "warning");
        return {
          restored: false,
          needsManualRecovery: false,
          message: cleanMsg,
          resolution: "missing-stash-clean-tree",
        };
      }
      logWarning("preflight", msg);
      notify(msg, "warning");
