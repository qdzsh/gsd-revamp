...
const trustedUserMcpServerNames = (() => {
	try {
		const path = join(gsdHome(), "agent", "mcp-trust.json");
		if (!existsSync(path)) return new Set<string>();
		const data = JSON.parse(readFileSync(path, "utf-8")) as { servers?: unknown };
...
async function assertTrustedStdioServer(
	...
	if (
		config.sourcePath === join(gsdHome(), "mcp.json") &&
		trustedUserMcpServerNames.has(config.name)
	) {
		return undefined;
	}
