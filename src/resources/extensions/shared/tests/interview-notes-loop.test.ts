// GSD2 — Regression test for interview-ui "Type something." notes loop
// Copyright (c) 2026 Jeremy McSpadden <jeremy@fluxlabs.net>

/**
 * Regression test for bug #3502:
 *
 * Selecting "Type something." opens the notes field, but pressing Enter
 * after typing a note called goNextOrSubmit() which saw the cursor still
 * on the "Type something." slot and re-opened notes — trapping the user
 * in an infinite loop.
 *
...
describe("interview-ui notes loop regression (#3502)", () => {
	...
	it("does not loop when Enter is pressed after typing a note on 'Type something.'", async () => {
		// With 2 options, "Type something." is index 2 (0-based)
		// Cursor starts at 0, so press Down twice to reach it
		const result = await runWithInputs(questions, [
			DOWN,        // cursor → index 1 (CLI Tool)
			DOWN,        // cursor → index 2 (Type something.)
		...
		assert.equal(answer.selected, "Type something.");
	});

	it("Enter on empty notes advances instead of re-opening (notesVisible guard)", async () => {
		// Press Down twice to "Type something.", Enter to select
	...
	it("Chat about this exits the structured question without answers", async () => {
		const result = await runWithInputs(questions, [
			DOWN,        // cursor → CLI Tool
			DOWN,        // cursor → Type something.
			DOWN,        // cursor → Chat about this
			ENTER,
		]);
