...
function exitIfManagedResourcesAreNewer(currentAgentDir: string): void {
  ...
  process.stderr.write(
    ...
    `[gsd] Run ${chalk.bold('npm install -g gsd-revamp@latest')} or ${chalk.bold('gsd upgrade')}, then try again.\n`,
  )
  process.exit(1)
}
...
if (cliFlags.messages[0] === 'graph') {
  ...
  const { buildGraph, writeGraph, graphStatus, graphQuery, graphDiff, resolveGsdRoot } = await import('@gsd-revamp/mcp-server')
