...
describe('symlink-numbered-variants', async () => {
  const base = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-symlink-variants-")));
  const stateDir = realpathSync.native(mkdtempSync(join(tmpdir(), "gsd-state-variants-")));
