...
const bundledGsdHomeDir = existsSync(join(resourcesDir, 'gsd-home'))
  ? join(resourcesDir, 'gsd-home')
  : join(packageRoot, 'src', 'resources', 'gsd-home')
...
function getBundledPackageName(): string {
  try {
    const pkg = JSON.parse(readFileSync(join(packageRoot, 'package.json'), 'utf-8'))
    return typeof pkg?.name === 'string' ? pkg.name : 'gsd-revamp'
  } catch {
    return 'gsd-revamp'
  }
}
...
function getNextBackupPath(path: string, stamp: string): string {
  const base = `${path}.bk.${stamp}`
  ...
  throw new Error(`Could not allocate backup path for ${path}`)
}

function pathsHaveSameFileContent(left: string, right: string): boolean {
  try {
...
function backupExistingPath(path: string, stamp: string): void {
  if (!existsSync(path)) return

  const backupPath = getNextBackupPath(path, stamp)
  const stats = lstatSync(path)
  if (stats.isDirectory()) {
    cpSync(path, backupPath, { recursive: true, force: false })
  } else {
    copyFileSync(path, backupPath)
  }
}

function syncGsdHomeFileWithBackup(src: string, dest: string, stamp: string): void {
  if (!existsSync(src)) return

  mkdirSync(dirname(dest), { recursive: true })
  if (existsSync(dest)) {
    const destStats = lstatSync(dest)
    if (destStats.isFile() && pathsHaveSameFileContent(src, dest)) return

    backupExistingPath(dest, stamp)
...
function syncGsdHomeDirWithBackup(srcDir: string, destDir: string, stamp: string): void {
  if (!existsSync(srcDir)) return

  if (existsSync(destDir)) {
    const destStats = lstatSync(destDir)
    if (!destStats.isDirectory()) {
      backupExistingPath(destDir, stamp)
      if (destStats.isSymbolicLink()) unlinkSync(destDir)
      else rmSync(destDir, { force: true })
    }
  }
  ...
  for (const entry of readdirSync(srcDir, { withFileTypes: true })) {
    ...
    try {
      if (entry.isDirectory()) {
        syncGsdHomeDirWithBackup(srcPath, destPath, stamp)
      } else {
        syncGsdHomeFileWithBackup(srcPath, destPath, stamp)
      }
    } catch {
      // Non-fatal per path; preserve blocked config and continue syncing others.
    }
  }
}

function syncGsdHomeDefaults(agentDir: string): void {
  if (!existsSync(bundledGsdHomeDir)) return
  const gsdHomeDir = dirname(agentDir)
  const backupStamp = createBackupStamp()
  mkdirSync(gsdHomeDir, { recursive: true })

  for (const file of ['mcp.json', 'PREFERENCES.md']) {
    const src = join(bundledGsdHomeDir, file)
    if (!existsSync(src)) continue
    try {
      syncGsdHomeFileWithBackup(src, join(gsdHomeDir, file), backupStamp)
    } catch {
      // Non-fatal; preserve user config if backup or overwrite is blocked.
    }
  }

  const agentDefaultsDir = join(bundledGsdHomeDir, 'agent')
  if (existsSync(agentDefaultsDir)) {
    try {
      syncGsdHomeDirWithBackup(agentDefaultsDir, agentDir, backupStamp)
    } catch {
      // Non-fatal; preserve user config if backup or overwrite is blocked.
    }
    ...
    if (existsSync(binDir)) {
      for (const entry of readdirSync(binDir, { withFileTypes: true })) {
        ...
        try {
          ...
          // chmod is best-effort on Windows and read-only filesystems.
        }
      }
    }
  }
