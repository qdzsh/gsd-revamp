...
test("forensics prompt requires gh CLI with --repo qdzsh/gsd-revamp for issue creation", () => {
  const prompt = readPrompt("forensics");

  // Must contain the exact gh CLI command with the correct repo flag
  assert.match(
    prompt,
    /gh issue create --repo qdzsh\/gsd-revamp/,
    "Prompt must specify gh issue create --repo qdzsh/gsd-revamp",
  );
});
