...
function makeFixture(): Fixture {
  const projectDir = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-decoupled-')));
  mkdirSync(join(projectDir, '.gsd'), { recursive: true });

  const fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-decoupled-home-')));
