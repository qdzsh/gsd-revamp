...
// ─── Bug 2: success notifications render with success theme ─────────────

describe("Success-notification rendering — success theme without borders", () => {
	...
	it("renders the message without horizontal borders", () => {
		const c = buildSuccessNotification("Milestone M042 ready.");
		const lines = c.render(40);
		const joined = lines.map(stripAnsi).join("\n");
		assert.match(joined, /Milestone M042 ready\./);
		assert.doesNotMatch(joined, /─/);
	});

	it("the message text appears in the rendered output", () => {
		const c = buildSuccessNotification("Milestone M042 ready.");
		const lines = c.render(40);

		const messageRow = lines.findIndex((l) => stripAnsi(l).includes("Milestone M042 ready."));
		assert.ok(messageRow >= 0, "message must appear");
	});

	it("message carries ANSI success styling", () => {
		const c = buildSuccessNotification("ok");
		const lines = c.render(40);
		const top = lines.find((line) => stripAnsi(line).includes("ok")) ?? "";
		assert.ok(
			top !== stripAnsi(top),
			"message must contain ANSI styling from theme.fg('success', ...)",
		);
	});
