 ...
 * Resolution order:
 *   1. @gsd-revamp/engine-{platform} npm optional dependency (production install)
...
function loadNative(): Record<string, unknown> {
  ...
  if (process.env.GSD_NATIVE_DISABLE === "1") {
    ...
    if (packageSuffix) {
      try {
        const loaded = _require(`@gsd-revamp/engine-${packageSuffix}`) as Record<string, unknown>;
        _loadedSuccessfully = true;
        return loaded;
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        errors.push(`@gsd-revamp/engine-${packageSuffix}: ${message}`);
      }
    }
