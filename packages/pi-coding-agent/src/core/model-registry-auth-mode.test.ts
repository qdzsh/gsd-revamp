...
describe("ModelRegistry authMode — getAvailable", () => {
	...
	it("excludes denylisted providers from available models", () => {
		const registry = createRegistry(() => true);
		registry.setDisabledModelProviders(["openrouter"]);
		const available = registry.getAvailable();
		assert.equal(
			available.some((m) => m.provider === "openrouter"),
			false,
			"openrouter models must be hidden when provider is denylisted",
		);
	});
...
describe("ModelRegistry authMode — getApiKey", () => {
	...
	it("still resolves provider keys for denylisted providers", async () => {
		const registry = createRegistry(
			() => true,
			async (provider: string) => provider === "openrouter" ? "sk-or-test-token" : undefined,
		);
		registry.setDisabledModelProviders(["openrouter"]);
		const key = await registry.getApiKeyForProvider("openrouter");
		assert.equal(key, "sk-or-test-token");
	});
});
