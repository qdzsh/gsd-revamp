...
function makeCtx(sessionId = "test-session") {
  const model = { provider: "openai-codex", id: "gpt-5.5", api: "responses", contextWindow: 128000 };
  return {
    ui: {
      notify: () => {},
      setStatus: () => {},
...
test("deep project setup: research-project blocker placeholder is a file, not the research directory", () => {
  ...
  try {
    ...
    assert.equal(expectedPath, join(realpathSync.native(base), ".gsd", "research", "PROJECT-RESEARCH-BLOCKER.md"));
