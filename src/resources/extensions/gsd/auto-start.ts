...
export async function bootstrapAutoSession(
  ...
  if (preferredModel) {
    ...
    if (match) {
      ...
      ctx.ui.notify(
        `Preferred model ${formatDisplayModel(preferredModel.id)} from PREFERENCES.md is not configured; falling back to session default.`,
        "warning",
      );
    }
  }
  ...
  try {
    ...
    if (workerModelOverride && process.env.GSD_PARALLEL_WORKER === "1") {
      ...
      if (overrideModel) {
        ...
        if (ok) {
          ...
          ctx.ui.notify(`Worker model override: ${formatDisplayModel(overrideModel.id)}`, "info");
        }
      }
    }
    ...
    const startModelLabel = s.autoModeStartModel
      ? formatDisplayModel(s.autoModeStartModel.id)
      : ctx.model ? formatDisplayModel(ctx.model.id) : "default";
    ...
    const effectiveCeiling = (routingConfig.enabled && routingConfig.tier_models?.heavy)
      ? formatDisplayModel(routingConfig.tier_models.heavy)
      : startModelLabel;
