...
/** Returns true for models that support the adaptive thinking API (Opus 4.6/4.7/4.8, Sonnet 4.6/4.7, Haiku 4.5). */
...
export function mapThinkingLevelToEffort(level: string | undefined, modelId: string): AnthropicEffort {
	switch (level) {
		...
		case "high":
			return "high";
		case "xhigh":
			if (modelId.includes("opus-4-8") || modelId.includes("opus-4.8")) return "xhigh";
