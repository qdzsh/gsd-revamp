...
test("disabled_model_providers validates and normalizes string arrays", () => {
  const { preferences, errors } = validatePreferences({
    disabled_model_providers: ["openrouter", "  openrouter  ", "openai-codex", "   "],
  });
  assert.equal(errors.length, 0);
  assert.deepEqual(preferences.disabled_model_providers, ["openrouter", "openai-codex"]);
});

test("disabled_model_providers rejects non-array values", () => {
  const { errors } = validatePreferences({ disabled_model_providers: "openrouter" as any });
