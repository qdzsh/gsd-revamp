...
test("buildSearchQuery scopes duplicate search to this repository", () => {
  assert.equal(
    buildSearchQuery("gsd-revamp", "gsd-revamp", {
      title: "Auto mode fails after merge conflict",
    }),
    "repo:qdzsh/gsd-revamp is:issue in:title auto mode fails after merge conflict",
  );
});
...
test("hasExistingDedupeCommentInIssue paginates until marker is found", async () => {
  ...
  const found = await hasExistingDedupeCommentInIssue(githubJson, "gsd-revamp", "gsd-revamp", 42);
