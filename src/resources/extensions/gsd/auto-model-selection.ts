...
function sanitizeThinkingPolicy(value: unknown): ThinkingPolicy | null {
  ...
  if (isThinkingLevel(raw.default)) {
    policy.default = raw.default;
  }
  if (raw.prefixes && typeof raw.prefixes === "object") {
    const prefixes: Record<string, ThinkingLevel> = {};
    for (const [prefix, level] of Object.entries(raw.prefixes as Record<string, unknown>)) {
      if (prefix && isThinkingLevel(level)) {
        prefixes[prefix] = level;
      }
    }
    if (Object.keys(prefixes).length > 0) policy.prefixes = prefixes;
  }
  if (raw.unitTypes && typeof raw.unitTypes === "object") {
    const unitTypes: Record<string, ThinkingLevel> = {};
    for (const [unitType, level] of Object.entries(raw.unitTypes as Record<string, unknown>)) {
      if (unitType && isThinkingLevel(level)) {
        unitTypes[unitType] = level;
      }
    }
    if (Object.keys(unitTypes).length > 0) policy.unitTypes = unitTypes;
  }
  return policy;
}

export function readThinkingPolicy(): ThinkingPolicy | null {
  try {
    const raw = readFileSync(join(gsdHome(), "agent", "thinking-policy.json"), "utf-8");
    return sanitizeThinkingPolicy(JSON.parse(raw));
  } catch {
    return null;
  }
}

export function resolvePolicyThinkingLevel(
  unitType: string | null | undefined,
  policy: ThinkingPolicy | null,
): ThinkingLevel | null {
  if (!unitType || !policy) return null;
  const exact = policy.unitTypes?.[unitType];
  if (exact) return exact;

  let bestPrefix = "";
  let bestLevel: ThinkingLevel | null = null;
  for (const [prefix, level] of Object.entries(policy.prefixes ?? {})) {
    if (unitType.startsWith(prefix) && prefix.length > bestPrefix.length) {
      bestPrefix = prefix;
      bestLevel = level;
    }
  }
...
export function resolvePreferredThinkingLevel(
  unitType: string | null | undefined,
  routingTier?: string | null,
  retryContext?: { isRetry?: boolean } | null,
  policy: ThinkingPolicy | null = readThinkingPolicy(),
): ThinkingLevel {
  const policyLevel = resolvePolicyThinkingLevel(unitType, policy);
  if (policyLevel) return policyLevel;

  if (
    unitType?.startsWith("research-")
    || unitType?.startsWith("discuss-")
    || unitType?.startsWith("plan-")
  ) {
    return "xhigh";
  }
  if (retryContext?.isRetry) return "xhigh";
  if (routingTier === "heavy") return "xhigh";
  return policy?.default ?? "medium";
}
...
export async function selectAndApplyModel(
  ...
  let selectedThinkingLevel: ThinkingLevel | null = null;
  ...
  if (modelConfig) {
    ...
    if (routingConfig.enabled) {
      ...
      if (shouldClassify) {
        ...
        selectedThinkingLevel = resolvePreferredThinkingLevel(unitType, classification.tier, retryContext);
      }
    }
    ...
    for (const modelId of modelsToTry) {
      ...
      if (policyAllowedModelKeys) {
        ...
        if (!policyAllowedModelKeys.has(key)) {
          if (verbose) {
            ctx.ui.notify(`Model policy denied ${formatDisplayModel(model.id)}; trying fallback.`, "warning");
          }
          continue;
        }
      ...
      if (isModelBlocked(basePath, model.provider, model.id)) {
        ctx.ui.notify(
          `Skipping blocked model ${formatDisplayModel(model.id)} (provider rejected it for this account).`,
          "warning",
        );
        continue;
      }
      ...
      if (ok) {
        appliedModel = model;
        const desiredThinkingLevel = selectedThinkingLevel
          ?? resolvePreferredThinkingLevel(unitType, routing?.tier, retryContext)
          ?? autoModeStartThinkingLevel;
        reapplyThinkingLevel(pi, desiredThinkingLevel);
        if (verbose && desiredThinkingLevel) {
          ctx.ui.notify(`Thinking: ${desiredThinkingLevel} for ${unitType}`, "info");
        }
        ...
        if (verbose) {
          const fallbackNote = modelId === effectiveModelConfig.primary
            ? ""
            : ` (fallback from ${formatDisplayModel(effectiveModelConfig.primary)})`;
          const phase = unitPhaseLabel(unitType);
          ctx.ui.notify(`Model [${phase}]${routingTierLabel}: ${formatDisplayModel(model.id)}${fallbackNote}`, "info");
        ...
        if (nextModel) {
          if (verbose) ctx.ui.notify(`Failed to set model ${formatDisplayModel(modelId)}, trying ${formatDisplayModel(nextModel)}...`, "info");
        } else {
    ...
    if (startBlocked) {
      ctx.ui.notify(
        `Auto-mode start model ${formatDisplayModel(autoModeStartModel.id)} is blocked for this account. Using current session model instead.`,
        "warning",
      );
    } else {
          ...
          if (byId) {
            ...
            if (fallbackOk) {
              ...
              reapplyThinkingLevel(
                pi,
                resolvePreferredThinkingLevel(unitType, null, retryContext) ?? autoModeStartThinkingLevel,
              );
            }
          }
        } else {
