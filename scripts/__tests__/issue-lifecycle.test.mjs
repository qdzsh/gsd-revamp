...
test("buildNeedsInfoSweepQuery selects stale open needs-info issues", () => {
  ...
  assert.equal(
    buildNeedsInfoSweepQuery("gsd-revamp", "gsd-revamp", 14, now),
    "repo:qdzsh/gsd-revamp is:issue is:open label:needs-info updated:<2026-05-03",
  );
});
...
test("listNeedsInfoSweepIssues paginates and excludes pull requests", async () => {
  ...
  const issues = await listNeedsInfoSweepIssues(githubJson, "gsd-revamp", "gsd-revamp", 14);
