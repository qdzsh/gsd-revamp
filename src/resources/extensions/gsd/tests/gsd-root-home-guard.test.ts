...
describe('gsdRoot() refuses ~/.gsd as project state when basePath is $HOME (#5187)', () => {
  ...
  beforeEach(() => {
    fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-home-guard-')));
  ...
  test('does NOT throw for an unrelated project directory that has its own .gsd', () => {
    const projectDir = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-home-guard-proj-')));
...
describe('git-root anchor guard: subdir basePath must not resolve to ~/.gsd', () => {
  ...
  beforeEach(() => {
    ...
    fakeHome = realpathSync.native(mkdtempSync(join(tmpdir(), 'gsd-anchor-guard-')));
