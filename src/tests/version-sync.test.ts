...
function createFixture(): string {
  const root = mkdtempSync(join(tmpdir(), "gsd-revamp-version-sync-"));
  writeJson(root, "package.json", { name: "gsd-revamp", version: "1.0.0" });
  writeJson(root, "package-lock.json", {
    name: "gsd-revamp",
    version: "1.0.0",
    packages: {
      "": { name: "gsd-revamp", version: "1.0.0" },
...
test("verifyVersionSync reports every release-owned surface that drifts from root", () => {
  ...
  writeJson(root, "package.json", { name: "gsd-revamp", version: "2.0.0" });
  ...
  assert.match(
    issues.join("\n"),
    /packages\/pi-coding-agent\/package\.json dependencies\.@gsd-revamp\/contracts is \^1\.0\.0, expected \^2\.0\.0/,
  );
