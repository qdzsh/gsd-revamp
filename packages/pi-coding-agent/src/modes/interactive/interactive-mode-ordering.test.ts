...
test("tool expansion applies to nested visible expandable components", () => {
	class ExpandableProbe extends Container {
	...
	const direct = new ExpandableProbe();
	const deep = new ExpandableProbe();
	root.addChild(direct);
	root.addChild(nested);
	nested.addChild(deep);

	const count = setExpandableDescendantsExpanded([root], true);

	assert.equal(count, 2);
	assert.equal(direct.expanded, true);
