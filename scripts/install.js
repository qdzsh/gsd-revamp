 ...
 *
 * Entry point for `npx gsd-revamp` or `npx gsd-revamp@latest`.
 ...
 * This script only fires for `npx gsd-revamp` (the package name bin).
 */
...
// packageRoot is always relative to this script — it's the gsd-revamp package directory.
// This is correct whether running as postinstall (inside node_modules/gsd-revamp) or
...
if (HAS_HELP) {
  ...
  ${c.yellow}Usage:${c.reset}
    npx gsd-revamp@latest          Install GSD globally (recommended)
    npx gsd-revamp@latest --local  Install GSD to current project
...
async function installGlobally() {
  startSpinner('Installing gsd-revamp globally...             ')
  try {
    const result = await new Promise((res) => {
      execCb(
        `npm install -g gsd-revamp@${gsdVersion}`,
        { timeout: 300_000 },
        (error, stdout, stderr) => {
    ...
    if (!result.ok) {
      const meaningful = (result.stderr || '')
        ...
        .slice(-3)
        .join('; ')
      printFail('Global install failed', meaningful || 'run npm install -g gsd-revamp manually')
      return false
    }

    printStep('Installed globally', 'npm install -g gsd-revamp')
    return true
  } catch (err) {
    stopSpinner()
...
async function installLocally() {
  startSpinner('Installing gsd-revamp locally...              ')
  try {
    const result = await new Promise((res) => {
      execCb(
        `npm install gsd-revamp@${gsdVersion}`,
    ...
    if (!result.ok) {
      const meaningful = (result.stderr || '')
        ...
        .slice(-3)
        .join('; ')
      printFail('Local install failed', meaningful || 'run npm install gsd-revamp manually')
      return false
    }

    printStep('Installed locally', 'npm install gsd-revamp')
    return true
  } catch (err) {
    stopSpinner()
...
async function downloadToFile(url, destination) {
  const response = await fetch(url, { headers: { 'User-Agent': 'gsd-revamp-installer' } })
...
async function installRtk() {
  ...
  try {
    const checksumsResponse = await fetch(`${releaseBase}/checksums.txt`, {
      headers: { 'User-Agent': 'gsd-revamp-installer' },
    })
